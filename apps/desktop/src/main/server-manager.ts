...
export interface CreateServerManagerOptions {
  ...
  secret: string;
  /**
   * §2.2 step 4's per-boot token, generated by the main process (which also puts
   * it in the window URL at step 8, so both ends must be handed the same one —
   * hence a parameter rather than something generated in here).
   */
  bootToken: string;
  /**
...
export interface ServerManager {
  ...
  /** The token handed to the child, mirrored for the §2.2 step 8 URL. */
...
class ServerManagerImpl implements ServerManager {
  ...
  get bootToken(): string {
    return this.#opts.bootToken;
  }
