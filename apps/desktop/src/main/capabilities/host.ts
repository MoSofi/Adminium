...
/**
 * CapabilityHost — the generic capability registry + IPC
 * router.
 *
 * The whole point is that the pipeline — manifest → consent → grant → IPC →
 ...
 * - the grant table belongs to the SERVER (the main process never opens the meta
 *  store), so the host does not read it itself — it is handed a {@link
 *  CapabilityGrantReader} that reaches the loopback REST API, exactly as
 *  `main/backup.ts`'s `BackupTransport` reaches `POST /desktop/ backup`. The
 *  real reader lives in `main/index.ts`; the unit suite injects a fake.
 *
 ...
 * dashboard writes grants straight to the server). Invoke is already async and
 * rare (a user pressing "print a receipt"), so a fresh loopback read is
 * negligible and always correct — there is no stale window to reason about.
 *
 * WHY THE CHECK IS BY `capabilityId`, not by manifest. The frozen bridge
...
/**
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │ FROZEN. The two interfaces below are quoted verbatim and are being      │
 * │ frozen together with the manifest `capabilities` field, so changing     │
 * │ their shape is a cross-document decision, not a refactor.               │
 * └─────────────────────────────────────────────────────────────────────────┘
 */
...
/** The typed rejections. The SPA branches on these, so they are part of the
 * API. */
...
/**
 * One consented grant, as the host reads it. Mirrors — and is pinned to, by the
 * `import type` assertion at the bottom of this file — `@adminium/server`'s
 * `CapabilityGrant` (which is `@adminium/meta`'s). Restated locally rather than
 * imported so this frozen-interface module stays free of a runtime edge that would
 * pull Fastify into the main process; the assertion fails the typecheck the moment
 * the two drift, the same arrangement `backup-archive.ts` uses.
 */
...
/**
 * Reads the current grant table. `null` is the honest answer when there is no
 * session to read AS — first run, or "Require login on this device" with nobody
 * signed in — and the host treats it as "no grants", so a call fails CLOSED
 * (`CAPABILITY_NOT_GRANTED`), the only safe direction for a gate.
 */
...
export interface CreateCapabilityHostOptions {
  ...
  /** The main-process log — refusals are recorded, never swallowed. */
  log?: ((line: string) => void) | undefined;
}

/**
 * The real host: holds providers, and gates every `invoke` on a fresh read of
 * the grant table.
 */
  ...
  return {
    ...
    async invoke(capabilityId: string, method: string, payload: unknown): Promise<unknown> {
      ...
      if (provider === undefined) {
        ...
        // code: it maps to INTERNAL, which is right, because it is a build
      ...
      // The provider owns the rest — including `CAPABILITY_STUB` rejection,
      // which the escpos stub throws for `print`. It surfaces unchanged: the ipc
      // layer maps the message prefix to typed code.
