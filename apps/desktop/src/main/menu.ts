...
/**
 * Native application menu. Structure owned by wires the localization.
 *
 * ─── How this menu is localized, and why not by importing @adminium/i18n ─────
 *
 * "All labels localized via @adminium/i18n bundles at menu build time; menu
 ...
 * SPA (ThemeProvider owns that axis), not `app.getLocale()` — only the renderer
 * knows it, and only the renderer knows when it changes.
 *
 * So the renderer resolves these labels through its own i18n and pushes the
 * result over `setMenuLabels` ({@link DesktopMenuLabels}); `main/index.ts`
 * rebuilds the menu from them. This module holds ZERO translations beyond the
 * en-US fallback below, which is the boot menu (there is no SPA yet step
 ...
 *
 * The menu has two rules that are easy to miss and expensive to get wrong, and both
...
// ─── Labels ("All labels localized via @adminium/i18n") ──────────────────────
...
/**
 * en-US, and the fallback for {@link buildAppMenu} until lands. Ellipses are
 * the single-character U+2026, and the ones that carry it are exactly the items
 * that open a further dialog — a platform convention on every OS we ship.
 */
 ...
 * ({@link DesktopMenuLabels}, `setMenuLabels`), with the en-US fallback
 * behind every key.
 *
 * `null`/`undefined` labels — the boot menu, before the SPA has pushed anything
 * — collapse to the en-US default. A partial map falls back per-key, so a
 * locale bundle that is missing one string shows that one item in en-US rather
 * than blanking it; the parity gate (`packages/i18n`) makes a missing key a red
 * test long before it reaches here, but a native menu with a blank item is a
 * worse failure than a briefly-English one, so the fallback stays.
 */
 ...
 * File is wizard flows backup coordinator, Help is updater About panel — so
 * they arrive as callbacks and this module holds no opinion about any of them.
 * All optional: an unwired handler renders its item DISABLED rather than
 * absent, so a half-built menu looks like a feature that is not ready instead
...
/**
 * "Adminium Docs ↗" opens the system browser. `https:` only,
 ...
 *
 * No `/docs` suffix: the site lives AT `docs.adminium.dev`, so
 ...
 * renderer and dashboard builds, never `out/main`, so nothing here is checked
 * against its allowlist. An earlier version of this comment claimed it was,
 * which is how `adminium.io` — a host the scanner does not allow and would have
 * failed on sight — survived in the shipped Help menu.
 */
...
export const RESERVED_SPA_ACCELERATORS: readonly string[] = [
  // The command palette. "⌘K is deliberately NOT a native accelerator."
  'CmdOrCtrl+K',
  // Inline edit.
  'CmdOrCtrl+E',
];
...
/**
 * The menu bar: File · Edit · View · Window · Help, plus the macOS app menu.
 *
 * Pure — it builds the template and installs nothing, which is what lets the
 * unit suite assert the two rules without an Electron instance.
 */
export function appMenuTemplate(opts: BuildAppMenuOptions): MenuItemConstructorOptions[] {
  ...
  // opens Electron's own about panel, specifies OURS (app + server +
  ...
  template.push({
    ...
    submenu: [
      ...
      { type: 'separator' },
      // "Close Window / Quit". On mac Quit lives in the app menu above and
  ...
  template.push({
    ...
    submenu: [
      {
        // This renders as "Adminium Docs ↗"; the arrow is the affordance for
        // the rule that it leaves the app for the system browser.
        label: `${t('help.docs')} ↗`,
        click: () => {
          openExternal(DOCS_URL);
        },
      },
      // "opens the in-app Shortcuts Panel" — the SPA's own ⌘/ surface, so no
      // accelerator here; the panel already owns one.
...
// ─── Introspection (for the accelerator test) ────────────────────────────────
