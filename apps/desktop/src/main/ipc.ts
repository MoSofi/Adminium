...
/**
 * Typed `ipcMain` handlers — the main-process half of the preload bridge.
 *
 * The contract is `AdminiumDesktopApi`, one channel per method
 ...
 * zod, never cast. That is not ceremony. `contextIsolation` + `sandbox` exist
 * because the renderer is the one process an attacker reaches first, and these
 * handlers are the only doors out of it that run with main-process authority —
 * native dialogs, `config.json` writes, `quitAndInstall`.
 *
 ...
 *    `code`, and every custom property. The typed rejections
 ...
 * 2. **This file holds no business logic.** (the shell contains none)
 * ("Everything else the SPA needs comes from the server REST API"). Every
 * handler is: parse → call an injected port → wrap. A handler that grew a
 * decision would be invisible to self-host, where the same SPA runs against
 * the same server with no bridge at all.
 *
 ...
 * and the window manager (lists "BackupCoordinator (dialogs + shell reveal)";
 * they own the parent window, the per-kind file filters and the localization of
 * the filter labels), the updater to, the CapabilityHost to, the LAN URLs to.
 * Routing is this file's whole job.
 *
 * WIRING REQUIREMENT: `registerIpcHandlers()` must run BEFORE the first
 * BrowserWindow is created. The preload reads `platform`/`versions` with a
...
/**
 * A FILENAME, not a path (`defaultName`). Separators are rejected because
 ...
 *
 * Absolute-only, and NOT confined to `dataDir`: the About panel reveals the
 * data directory, reveals a backup archive, and Help → "Show logs" reveals
 ...
 * "updates" | "telemetryOptIn" | "autoBackup">>`, reusing schemas so the
 * bounds (a valid port, `keep` within 1–365) are stated once.
 *
...
/**
 * `setDataDir`. The one key `setConfigSchema` above exists to refuse,
...
/**
 * `setMenuLabels`: the native menu's localized strings, pushed by the
 ...
 * untrusted boundary. The keys are stated once here and pinned to the contract
 * by {@link _MenuLabelsMatchSchema}; drift becomes a desktop typecheck failure
 * rather than a menu the main process quietly leaves in English. The `.max` is
 * generous — a menu label is a few words in any locale — and exists only to
 * bound what a compromised renderer can push into a native widget.
 */
...
const capabilityInvokeSchema = z.strictObject({
  ...
  /**
   * The payload is `unknown` by contract — the provider owns its shape, and a
   ...
   * It does — but has methods that take no payload at all (`listDevices`), so
   * "absent" is a legitimate call and not a serialization detail to bet on.
   */
...
// `node:fs` and no Electron — see that file's header), so the shapes it
...
/**
 * The `setMenuLabels` schema accepts exactly `DesktopMenuLabels`.
 *
...
/**
 * The runtime facts, as `getRuntimeInfo` needs them.
 *
 ...
 * the SPA cannot ask before, but a crash page exists precisely when the server
 * does not.
 */
export interface DesktopRuntimeSnapshot {
  ...
  /**
   * Has the env kill-switch (`ADMINIUM_DISABLE_UPDATES=1`) forced updates off
   * for THIS launch? A boot-time fact, resolved by `main/index.ts` from
   ...
   * and built no updater at all. The panel would misrepresent the very air-gap
   * posture that admin set, and the button would fail closed to UNAVAILABLE,
...
/**
 * The native dialogs the backup channels need.
 *
 * Injected, not implemented here: they belong to the BackupCoordinator
 * ("dialogs + shell reveal") and the window manager, which own the parent
 * window, the per-kind file filters, and the localization of the filter
 * labels. This module knows only that a dialog returns an absolute path, or
...
export interface RegisterIpcHandlersOptions {
  ...
  ipc: IpcMainLike;
  /** `platform` + `versions`, answered synchronously at preload time. */
  bootstrap: BridgeBootstrap;
  /** Push to the window's `webContents` — `onUpdateEvent`. */
  broadcast: (channel: string, payload: unknown) => void;
  runtime: () => DesktopRuntimeSnapshot | null;
  /** The live `config.json`. */
  readConfig: () => DesktopConfig;
  /** Merge + persist a patch. The atomic write is `saveConfig`. */
  writeConfig: (patch: DesktopConfigPatch) => Promise<void>;
  /**
   * The data-directory change, gate included.
   *
   * A port rather than logic here, because every part of the act belongs to
   * `main/index.ts`: `detectCloudSyncFolder` is the gate, `config.save`
   * is the write, and `app.relaunch()` is what makes the new directory the one
   * the server is forked against. This module's job is the same as for every
   * other channel — parse the payload, hand it over, wrap the answer.
   */
  setDataDir: (opts: SetDataDirOptions) => Promise<SetDataDirResult>;
  dialogs: DesktopDialogs;
  /**
   * A GETTER for the live updater, `null` when `updates.mode: "disabled"` or
   * `ADMINIUM_DISABLE_UPDATES=1`. Nullable rather than a no-op manager because
   * requires the updater to never be INITIALIZED in that mode — "not
  ...
  /**
   * Apply the renderer's localized menu labels — rebuild the native menu.
   *
   * A port because the menu is `main/index.ts`'s to (re)install: it owns the
   * `MenuHandlers` (backup, Help's logs) and the platform/dev flags, and it
  ...
  /**
   * The diagnostic blob's "dataDir size". A port because only the main
   * process can walk the data directory — the renderer has no filesystem
   * access, and the server owns the data but not its on-disk footprint. `main/
   * index.ts` supplies the walk; this file parses, calls, and wraps.
   */
  getDiagnostics: () => Promise<DesktopDiagnostics>;
  /**
   * The in-app licence viewers. A port because the two files live inside the
  ...
  /** The reachable `http://<LAN-IPv4>:<port>` list. Defaults to none. */
  lanShareUrls?: (() => readonly string[]) | undefined;
  /** Help → "Show logs": reveal `<userData>/logs`. */
...
export interface IpcHandlers {
  /** `onUpdateEvent`. The updater calls this; nothing else does. */
...
/** 127.0.0.0/8,::1, localhost — the same peers boot-token route accepts. */
 ...
 *
 * - `file:` — the bundled splash and crash pages (steps 6 and 9). The crash page
 *  must reach `showLogs()` at the precise moment there is no server and
 *  therefore no loopback origin to compare against.
 * - any loopback `http:`/`https:` origin — the server on its random `:0` port,
 *  and the dashboard's Vite dev server in the dev loop. Pinning to
 *  `127.0.0.1:<serverPort>` would be tighter by nothing: the navigation
...
/**
 * The port collision, as a code the settings form can branch on.
 *
 ...
 * closed union, and it recognises prefixes — the same convention providers use
 * (`printer-escpos.ts` rejects with `` `${CAPABILITY_STUB}: …` ``). A class would
 * work here and nowhere else, since the thrower is
...
/**
 * Map a thrown error onto closed code union.
 *
 * The providers reject with `` `${CAPABILITY_NOT_GRANTED}: …` `` — a code
 * inside a message, which is how that section specifies it and how
 * `printer-escpos.ts` implements it. `LAN_PORT_IN_USE` joins them on the
 ...
 * a stack in the renderer is an information leak, and the log has the real one.
 */
...
/**
 * Wire every channel. `ipcMain.handle` throws on a duplicate channel, so a
...
export function registerIpcHandlers(opts: RegisterIpcHandlersOptions): IpcHandlers {
  ...
  // The two synchronous properties. `sendSync` because `contextBridge` copies
  ...
  register(IPC_CHANNELS.getRuntimeInfo, noPayloadSchema, (): Promise<DesktopRuntimeInfo> => {
    ...
    return Promise.resolve({
      ...
      lanShare: {
        ...
        // that did not ask to share them is not a neutral act.
        urls: config.lanShare.enabled ? [...lanShareUrls()] : [],
      },
      // The mode the updater ACTUALLY runs in. `config.updates.mode` is the live
      // value the settings panel writes (so notify↔manual stay honest as the user
      // toggles them), but the env kill-switch overrides it to `disabled` without
      // touching the file — `createUpdateManager` resolved the same override and
      // built no updater, so reporting the raw config here would paint "Notify"
      // over an install that makes zero update traffic.
