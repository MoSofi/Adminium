 ...
 * and the auto-backup schedule.
 *
 * SCOPE, ("The server is the only data owner"): this module runs DIALOGS, not
 * backups. The archive itself is produced inside the server (`POST
 * /api/v1/desktop/backup`), which already holds the connections and can
 ...
 * a second one.
 *
 * And: a restore never deletes. Current data moves to
 * `<dataDir>/pre-restore-<ts>/` and stays there for the user
 * to empty.
 *
 * ─── How this process talks to the server, and why it is the user's session ──
 *
 * The backup route is session-guarded. Main has no cookie jar of its own — but
 * it OWNS the one the window uses: Electron's `session` holds the
 * `adminium_session` cookie the SPA got boot-token exchange, and main can read
 * it. So {@link BackupTransport} is "fetch, with the window's session cookie
 * attached", and the backup runs AS THE SIGNED-IN USER. That is not a
 * workaround, it is the correct principal: the audit row names a person, the
 * completion notification has an inbox to land in (`user_id` is NOT NULL),
 * "Require login on this device" keeps meaning something — with no session,
 ...
 * is not importable from the shell (/ `.dependency-cruiser.cjs`
 * `desktop-shell-only`), is the track that wires localized main- process strings.
 * The shape here is already a lookup + a `t` hook, so that change is a different
 * `t`, not a different module.
 */
...
/**
 * en-US, and the fallback until.
 *
 * `restore.confirm.body` is sentence, verbatim and non-negotiable: it is the
 * one screen where a user decides whether to replace their data, and the
 ...
 * on every restart (re-forks on a new:0 port) — neither of which a unit test
 * can supply and both of which the boot sequence already tracks.
 *
...
/** The native dialogs, narrowed to what uses (`main/window.ts` implements). */
...
/** The slice of `ServerManager` a restore drives (stop → unpack → start). */
export interface BackupServerControl {
  stop(): Promise<void>;
  start(): Promise<{ metaVersion: string }>;
  /**
   * The newest migration THIS app ships, from the handshake, or `null` if
   * no server has come up this boot. See `validateArchive`'s
...
/**
 * The "first idle moment after 03:00", as a port.
 *
...
/**
 * Create the schedule. Injected for the same reason `server-manager.ts` injects
 * `fork` and `timers`: the only other way to observe a 03:00 tick is to wait
 * until 03:00.
 */
...
export interface CreateBackupCoordinatorOptions {
  /** The live `config.json` — `autoBackup`, and the body's redacted copy. */
  ...
  /** `dataDir` — where a restore unpacks and moves data aside. */
  dataDir: string;
  /** The "first idle moment". Omitted ⇒ the schedule fires at 03:00 flat. */
 ...
 *
 * Injected so restore ORDER is assertable. That order — stop, move aside,
 ...
 * job and not a thing that runs on every commit.
 *
...
export interface BackupCoordinator {
  ...
  /** File → "Restore from backup…", and the step-1 launch-argument path. */
  restoreFrom(zipPath: string): Promise<void>;
  /** The daily auto-backup. Starts the schedule; returns a stop function. */
...
export function createBackupCoordinator(
  ...
  async function restoreFrom(zipPath: string): Promise<void> {
    await exclusive('restoreFrom', async () => {
      ...
      try {
        // Read-only, before anything is touched. This is the whole reason the
        // flow has a validation phase — every refusal below happens while the
        // user's data is still exactly where it was.
      ...
      const confirmed = await opts.dialogs.confirm({
        ...
        // The sentence. The path is interpolated because a promise about where
  ...
  /**
   * The flow, past the point of no return: stop → move aside → unpack → start.
   *
   ...
   * runs the migration ledger over whatever `meta.db` it now finds ("the migration
   * runner fast-forwards an older backup"), which needs no code here — only the
   * ordering.
   */
  ...
  /**
   * The auto-backup: "daily at first idle moment after 03:00 via croner",
   * rotating to `autoBackup.keep`.
   *
   * WHY THE SCHEDULE IS HERE AND THE WORK IS NOT. The server owns the archive
   * and it has a croner scheduler of its own — so a reader is right to ask why
   * this is not a server schedule like telemetry's. Two reasons, and the
   * second is decisive:
   *
   *  1. `autoBackup.enabled`/`keep` live in `config.json`, which the main
   * process owns and the child cannot read. A server-side schedule would need
   *     a whole mirror channel (the `ADMINIUM_DESKTOP_SINGLE_USER` dance) to
   *     learn two numbers.
   ...
   * draws — "BackupCoordinator (dialogs + shell reveal; work runs in server)".
   */
  ...
  async function runScheduledBackup(): Promise<void> {
    ...
    await exclusive('scheduled backup', async () => {
      ...
      try {
        ...
        // modal in front of a laptop that someone opens at 09:00 — the schedule routes
  ...
  /**
   * The "first idle moment after 03:00".
   *
...
/**
 * "daily at 03:00". LOCAL time, unlike the server's UTC schedules — the
...
/** "Idle": no input for this long. */
export const IDLE_THRESHOLD_SECONDS = 60;

/** `adminium-backup-20260712-1430.zip` — the archive name, for the save dialog. */
