 ...
 * which is what lets two rules be asserted without an Electron instance to hang
 * a menu on. Both rules fail SILENTLY in production — a missing Edit role means
 * ⌘C does nothing on macOS and throws nothing anywhere, and an accelerator
...
describe('appMenuTemplate', () => {
  it.each(PLATFORMS)('has \'s five menus on %s', (platform) => {
    const built = template({ platform });
    const labels = built.map((item) => item.label);
    // "File (…) · Edit (…) · View (…) · Window (standard) · Help (…)".
...
// ───: the Edit menu must use the standard roles ──────────────────────────────

describe('the Edit menu', () => {
  it.each(PLATFORMS)('uses the native editing roles on %s', (platform) => {
    // "Edit (standard roles — required for ⌘C/⌘V on mac)". Roles are how
...
// ───: no collision with the SPA's keyboard vocabulary ────────────────────────

describe('accelerators', () => {
  it.each(PLATFORMS)('claims none of the SPA\'s shortcuts on %s', (platform) => {
    // "⌘K is deliberately NOT a native accelerator." A native accelerator WINS
    // — it fires in the main process and the keystroke never reaches the page —
    // so a collision does not double-fire, it silently deletes an in-app
...
describe('the File menu', () => {
  it('offers \'s four commands plus Close', () => {
  ...
  it('puts Quit in the File menu everywhere except macOS', () => {
    ...
    // apps do, "Close Window / Quit" is the cross-platform shape.
...
describe('the Help menu', () => {
  // The docs site's real origin (`apps/docs/astro.config.mjs`
  ...
  // outside check-offline-assets.mjs's roots, so this literal is the only
  // gate on that value.
  const DOCS_ORIGIN = 'https://docs.adminium.dev';

  it('opens the docs in the system browser over https', () => {
    // "Adminium Docs ↗": external links go to the system
    // browser, https only.
  ...
  it('gives the Shortcuts Panel no accelerator', () => {
    // It "opens the in-app Shortcuts Panel" — a surface the SPA already
    // binds a key to. A native accelerator here would take that key away
    // from it.
  ...
  it('puts About where each platform puts it', () => {
    // mac: the app menu, per the platform convention. Elsewhere: Help.
  ...
  it('never uses Electron\'s built-in about panel', () => {
    // `role: 'about'` opens Electron's default panel; ours is specified (app +
...
// ─── The labels hook ("All labels localized via @adminium/i18n") ─────────────

describe('the labels hook', () => {
  it('renders every label through t()', () => {
    // A later wave swaps in an i18n-backed `t` and rebuilds on locale change. This
  ...
  it('has an en-US string for every key it can be asked for', () => {
    // The boot menu (before the SPA pushes a locale) and the
...
// ─── menuTranslator (labels pushed by the renderer, rebuilt per locale) ──

describe('menuTranslator (locale rebuild)', () => {
