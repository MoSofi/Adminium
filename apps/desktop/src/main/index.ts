 ...
 * it owns nothing else: `config.ts` owns `config.json`, `server-manager.ts` owns
 * the utilityProcess and its restart policy, `window.ts` owns the window and its
 * navigation lockdown. What lives here is
 * the choreography between them, plus the two app-lifecycle rules that have
 * nowhere else to go — the single-instance lock and the macOS activate /
 * window-all-closed conventions.
 *
 ...
 * drives the sequence with fakes; the Playwright `_electron` suite covers
 * the real shell.
 */
...
/**
 * Files a launch argument may name. `.zip` is the backup archive; the SQLite
 * extensions are the ones "Open an existing SQLite file" dialog accepts, so a
 * double-click behaves like that dialog.
 */
 ...
 *
 * Pure, because argv is the one part of with real edge cases and no way to
 * reproduce them by hand: Electron hands the second instance a full argv whose
 * head is the executable path and whose middle may carry Chromium switches
...
export interface AppUrlOptions {
  host: string;
  port: number;
  /** No `config.json` ⇒ wizard rather than the app. */
  firstRun: boolean;
  /**
   * Present only while `config.singleUser` is on — with it off the
   ...
   *
   * Exists for exactly one caller — LAN toggle ({@link LAN_SHARE_PATH}) — and
   * the constraint on it is that a MAIN-PROCESS CONSTANT is the only legal
   * argument. This string becomes the window's URL, and the window's URL is what
   * navigation lockdown exists to control; a path that could be sourced from the
   * renderer would let the untrusted side choose where the trusted side
...
/**
 * The URL the window loads once the server is ready.
 *
 * ALWAYS loopback, even when the server bound `0.0.0.0` because LAN share is
 * on. The window is a local client: navigation is locked to the loopback
 * origin, has the boot-token exchange reject non-loopback peers unconditionally
 * — so a window that addressed itself over the LAN interface would be refused
...
/**
 * The two preconditions for putting a token in the window URL, in one place:
...
/**
 * Where toggle sends the window after the rebind — the panel the user was
 ...
 * therefore the origin ("Fixed port ⇒ stable URLs";: loopback shares are
 * ephemeral). Landing on `/` would be the mechanically correct thing and the
...
/**
 * Where native "About Adminium" menu item sends the window. The same `/about`
 * route the Settings → About panel uses — "the native menu item opens the same
 * panel" — so there is one About surface, not two.
 *
 * A MAIN-PROCESS CONSTANT for the reason {@link AppUrlOptions.path} demands one:
 * this string becomes the window's URL, navigation lockdown exists to control
 * that. Sourcing it from the renderer would let the untrusted side pick where the
 * trusted side navigates. The navigation is a full load (no boot token — the
 * session cookie is already set), which is why it works from any app state and
 * cannot silently break the way a "menu → push → SPA subscriber" chain can if the
 * subscriber is ever unmounted.
 */
export const ABOUT_PATH = '/about';

// ─── LAN share ───────────────────────────────────────────────────────────────

/** A bind for the server child: its `ADMINIUM_HOST` + `ADMINIUM_PORT`. */
...
/**
 * The bind `config.lanShare` implies.
 *
 ...
 * (login toggle, update mode, backup schedule all write this same file) and none
 * of which has any business restarting the server. A port edit while sharing is
 * OFF is also `null`: nothing is bound to the old port, so there is nothing to
 * rebind, and the new number simply takes effect the next time the toggle goes
 * on.
 */
...
/**
 * Electron's `app`, narrowed to what need. A port rather than the real object
 * because `app` is a sprawling EventEmitter bound to a live Chromium: faking this
 * shape is a page of code; faking `app` is not possible.
 */
export interface DesktopAppHost {
  /** `process.argv` of THIS launch. */
  readonly argv: readonly string[];
  /** `darwin` | `win32` | `linux` — drives the window-all-closed convention. */
   ...
   * the child is still alive, which makes it the only place graceful shutdown
   * can happen — see {@link DesktopApp.start}'s quit hook.
   */
  onBeforeQuit(handler: (event: QuitEvent) => void): void;
  whenReady(): Promise<void>;
  quit(): void;
  /**
   * Queue a restart of this app, then end it — "relaunch", and the only way
   * data-directory change can take effect.
   *
   * On the host rather than a free function because it is app lifecycle, and
   * because the boot sequence is now its second caller: the `relaunch()` port
...
/** The config and the secret, as the boot sequence needs them. Backed by config.ts. */
export interface DesktopConfigPort {
   ...
   * warns on, persisting the config when resolution changed it (a freshly
  ...
  /**
   * Can the app actually use `dir` as its data directory? Creates it (and its
   * parents) when absent.
   *
  ...
  /**
   * Persist a config the bridge's `setConfig` has just patched (atomic write). Not
   * part of the boot sequence — the boot's own writes are decided inside {@link
   * DesktopConfigPort.load} / {@link DesktopConfigPort.resolveSecret} — but the
   * same file, so the same port owns it.
   */
  save(config: DesktopConfig): Promise<void>;
}

/**
 * The live state bridge reads and writes (`main/ipc.ts`'s ports).
 *
...
export interface DesktopBridgeContext {
  /** `null` until has run — `getRuntimeInfo` then answers UNAVAILABLE. */
  runtime: () => DesktopRuntimeSnapshot | null;
  /** The live `config.json`. Throws before step 2. */
  ...
  /** The data-directory change — gate, write, relaunch. */
  setDataDir: (opts: SetDataDirOptions) => Promise<SetDataDirResult>;
  /**
   * The renderer's localized menu labels. Stores them and rebuilds the native
   * menu (see {@link DesktopBootDeps.installMenu}). Registered with the bridge
   * like the other ports here, so `main/ipc.ts`'s `setMenuLabels` handler
...
export interface DesktopRuntimeState {
  ...
  /** Resolved by the handshake; `-1` until then. */
...
export interface DesktopBootDeps {
  ...
  /**
   * A factory rather than an instance, because the boot sequence only learns
   * the dataDir and the secret at steps 2–3.
   */
  createServerManager: (opts: CreateServerManagerOptions) => ServerManager;
  /** 32 random bytes, hex, per boot. Never persisted, never logged. */
  createBootToken: () => string;
  /**
   * The collision pre-flight: can the child bind `host:port` right now?
   *
  ...
  /** `<userData>/logs`; the manager rotates `adminium-server.log` in it. */
  logsDir: string;
  /** Absolute path to the forked bundle — `out/server/index.js`. */
  serverEntry: string;
  /**
   * The dashboard build, for `ADMINIUM_STATIC_ROOT`.
   *
   * The window navigates to `http://127.0.0.1:<port>/` and the embedded server
   * is what answers — so a server that was never told where the SPA is serves a
   * 404 and the app is a blank window over a healthy backend. Optional only
   * because the dev loop has no copied build and the server's own candidate
   * list finds `apps/dashboard/dist` instead; a packaged app has no such
   * fallback (copies the build to `out/dashboard`, and `@adminium/dashboard` is
   * a devDependency electron-builder prunes), which is exactly why omitting it
   * here was invisible in dev and fatal in production.
   */
  staticRoot?: string | undefined;
  /**
   * The wizard's demo card: the path to `resources/demo/demo-seed.mjs`, which the
   * server imports to seed the demo database.
   *
  ...
  /**
   * Wire `ipcMain` handlers — called ONCE, before the first window.
   *
   ...
   * because the preload reads `platform`/`versions` with a `sendSync` at load
   * time and THROWS if nothing answers, which fails the whole preload and leaves
   * `window.adminiumDesktop` undefined for the life of the window.
   */
  registerBridge: (context: DesktopBridgeContext) => void;
  /** Help → "Show logs" and the crash page's `logs` button: reveal
   * `<userData>/logs`. */
  showLogs: () => Promise<void>;
  /**
   * A FACTORY, for the same reason `createServerManager` is one: the coordinator
   * needs the `dataDir` (step 2) and the ServerManager (step 5), neither of
   * which exists when the deps are built.
   *
  ...
  /**
   * The native menu, (re)installed. A port because `Menu.setApplicationMenu`
   * needs a ready app and real Electron, and because the File items it wires
   * ("Back up now…" / "Restore from backup…") only have handlers once the
   * coordinator above exists — which is why this is CALLED from the boot
   * sequence rather than at module scope in `menu.ts`.
   *
   * Takes BOTH the handlers and the {@link MenuTranslate} because "menu
  ...
  /**
   * File → "Restore from backup…" — `openFile({ kind: 'backup' })`.
   *
   * Separate from the coordinator because the coordinator takes a PATH: the
  ...
  /**
   * Build the updater for this launch, or `null` when there must not be one. A
   * port — like `createServerManager` / `createBackup` — because the mode lives
   * in `config.json` (known at step 2) and the updater needs electron-updater
   * plus the ipc event sink, neither of which a unit suite can supply.
   *
   * `null` is the CORRECTNESS contract for `disabled`: the port resolves the
   * `ADMINIUM_DISABLE_UPDATES=1` override (a `process.env` fact this
  ...
  /**
   * Does the env kill-switch (`ADMINIUM_DISABLE_UPDATES=1`) force updates off for
   * this launch? A `process.env` fact this Electron-free sequence must not read
   * itself — the same reason `createUpdateManager` resolves the override behind
   * its own port — so it is injected as a boot constant and carried into the
   * runtime snapshot (`DesktopRuntimeSnapshot.updatesDisabledByEnv`) so About
...
export interface BackupWiring {
  ...
  readConfig: () => DesktopConfig;
  /** `dataDir`: where a restore unpacks and moves the old data aside. */
  dataDir: string;
  /** Stop/start/`metaVersion` — restore drives the child through these. */
   ...
   * DIFFERENT port after every restart, so a captured origin would be stale the
   * first time the LAN toggle flipped — and a backup posting to a dead port is a
   * failure the user sees as "backup broken".
   */
...
/**
 * Wire steps 1–9. The step numbers in the comments are the doc's, and keeping
 * them in the source is deliberate: the ORDER *is* the specification here, and
 * reordering it fails silently. Splash after the handshake is a blank window
 * for up to 30 s; the lock after `whenReady` is two servers writing one data
 * directory.
 *
...
export function createDesktopApp(deps: DesktopBootDeps): DesktopApp {
  ...
  /**
   * `null` until step 5, for the same reason `manager` is: the backup coordinator
   ...
   * `dispose()`s it so its 30 s / 24 h schedules cannot fire into a closing app
   * (the same reason `stopAutoBackup` lives at this scope).
   */
  let updateManager: UpdateManager | null = null;
  /**
   * Where the NEXT `ready` should land the window, consumed once.
   *
   ...
   * recovery, the 3-in-60 s policy's re-fork, toggle all arrive at it as the
   * same `ready`. Only the toggle knows the user was mid-flow on a particular
   * page, so only the toggle leaves a note; everything else finds `null` and
   * gets `/`, which is the right answer for a restart nobody asked for.
   */
  let pendingAppPath: string | null = null;

  /**
   * The menu state, held across rebuilds. `menuLabels` is the last locale the
   * renderer pushed (`null` ⇒ the en-US boot menu); `menuHandlers` is the
   * command set assembled at step 5. "The menu rebuilds on locale change" makes
   * install a repeated act — a push after step 5 rebuilds with the new labels
   * and the SAME handlers — so both go through {@link rebuildMenu}, the one
   * place that turns the pair into an installed native menu. A push that arrives
   * before step 5 (it cannot: the renderer that sends it only loads at step 8)
   * would still be safe — the empty `menuHandlers` gives a titles-only menu that
   * step 5 immediately supersedes.
   */
  ...
  /**
   * What to do with a file the OS handed us.
   *
   * A `.zip` is a backup and belongs to restore, which runs ENTIRELY in this
   * process — validate, confirm, stop, unpack, start — and needs no SPA at all.
   * That matters: `window.ts`'s `pendingFileArgument` note says delivering a
   * launch file to the renderer needs a new bridge method, and for a backup it
   * turns out nothing does. Everything else (`.sqlite`, `.db`, `.sqlite3`) is
   * "Open an existing SQLite file", which IS the wizard's, and is still held for
   * to collect.
   */
  ...
  /**
   * The toggle, end to end: "flipping the toggle updates `config.lanShare` …
   ...
   *
   * 1. **Probe before writing.** wants "an inline error with a 'Try 4601'
   * suggestion", and only a failure that changes NOTHING can be rendered inline:
   * the moment this function writes or restarts, the window navigates to a new
   * origin (step 4) and the renderer awaiting this call is gone, so an error
   * thrown after that point has nobody left to show it to. Probing first keeps
   * the overwhelmingly common failure — a port something else already holds —
   * inside one IPC round-trip with no side effects at all. See `lan.ts`'s
   * `probeBindable` for why this is a pre-flight and not a lock.
   ...
   * what the NEXT boot reads, so a crash in the window between the two would
   *    resolve to "silently sharing on the next launch".
  ...
  return {
    ...
    async start(): Promise<void> {
      ...
      // corruption cloud-sync warning exists to prevent — self-inflicted.
      ...
      // Single window; closing the last one quits everywhere but macOS,
      ...
      // The bridge, BEFORE anything can create a window — which is why it is
      ...
      // `window.adminiumDesktop` is undefined, every affordance is dead,
      // "Require login on this device" is unreachable. That includes the crash
      // screen below — a boot that fails at step 2 still opens a window.
      deps.registerBridge({
        runtime: () =>
          runtime === null
            ? null
            : {
                ...
                // A boot constant (the env var does not change mid-session), so
                // it is safe to read straight off the injected dep here — and it
                // makes `getRuntimeInfo` truthful the moment a runtime exists
        ...
        /**
         * The wizard's "Change…", end to end: gate, write, relaunch.
         *
         ...
         * `ADMINIUM_META_DSN` from it; the BackupCoordinator captured the same
         ...
         * which is precisely why says to choose NOW, and calls a later change "a
         * guarded operation (quit-and-move instructions)".
         *
         * The wizard survives it because the wizard was never the thing that
         * knew: `/desktop/setup` is gated server-side on "desktop runtime + zero
        ...
        setDataDir: async (input): Promise<SetDataDirResult> => {
          ...
          // The blocking warning, enforced HERE rather than in the renderer.
          // The renderer cannot see `detectCloudSyncFolder`, so this is the
          // only place the check can be a gate instead of a suggestion.
        ...
        // A locale push from the SPA. Store it and rebuild the native menu with
        // the same handlers — this is "the menu rebuilds on locale change", and
        // it is wired to the `setMenuLabels` channel through `ipc.ts`.
      ...
      // below, because those can fail — and the crash screen they open is a
      // real window with a real Quit button on it. Installed after the bridge
      // only because nothing can press a button before a window exists.
      //
      ...
      // The graceful shutdown, and the only hook from which it can happen.
...
/**
 * Merge `setConfig` patch into the live `config.json` body.
 *
...
/**
 * The port collision, as the code + message bridge carries.
 *
...
/**
 * A pre-`ready` failure. `ServerStartError` already carries the log excerpt and
 * path the crash page wants; anything else that got thrown gets its message
 * shown and nothing invented around it.
 */
...
/** `out/main/index.js` → `out/server/index.js` — the fork entry. */
 ...
 * `copyDashboardBuild()` puts the SPA.
 *
...
/**
 * `resources/demo/demo-seed.mjs` — the seed script the server imports to build
 * the demo database.
 *
 * `out/main/` → `././resources/demo/`, which resolves identically in dev (the
 * repo's `apps/desktop/resources/`) and in a packaged build (`files: [out/**,
 * resources/**]` keeps that relative layout inside the app).
 *
 * PACKAGING NOTE for: `resources/demo/**` must be listed in `asarUnpack`. The
 * server reaches this file with a real `import()`, and an asar-packed path is
 * not something the ESM loader can be relied on to open — unpacked, it is an
 * ordinary file on disk and the import is ordinary too.
 */
...
/**
 * `resources/add-ons-bundle/` — the pre-verified bundled add-on set: six
 * first-party tarballs + `.integrity` sidecars the desktop-release workflow
 ...
 * the same both-worlds reason: electron-builder's `files: [out/**, resources/**]` keeps the
...
/**
 * `out/main/` → `../../resources/` — the `LICENSE` and `THIRD-PARTY-NOTICES.txt`.
 *
 * The same `../../resources` arithmetic {@link bundledDemoSeedScript} uses, and
 * the same reason it holds in both dev and a packaged build: electron-builder's
...
/** A `readBundledText` kind → the file `generate-notices.mjs` stages. */
...
/**
 * The in-app licence viewers, over the packaged resources dir. `null` — not a
...
/**
 * "Copy diagnostic info … dataDir size" — the total bytes under `dir`,
...
/**
 * The backup transport: `POST /api/v1/desktop/backup`, carrying the WINDOW'S
 * session cookie.
 *
 * ─── Why the cookie, spelled out where it happens ────────────────────────────
 *
 * The route is session-guarded, and the main process has no session of its own —
 * but it owns the one the window uses. `session.defaultSession.cookies` is the
 * same jar the SPA's `adminium_session` cookie landed in when boot-token
 * exchange ran, and reading it here is what makes the backup run as THE
 * SIGNED-IN USER: the audit row names a person, `adminium_notifications` has an
 * inbox to land in (`user_id` is NOT NULL), "Require login on this device" keeps
 * meaning something. No session ⇒ `null` ⇒ the coordinator says "sign in first",
 * which is the right answer for an archive containing every row in the install.
 *
...
/**
 * The error envelope's `message`, or `null`.
 *
...
/**
 * The reply (`routes/desktop/schema.ts`).
 *
...
/**
 * The grant-list reply (`routes/desktop-capabilities/schema.ts`).
 *
...
/** The three build targets, `platform` narrows them. */
function desktopPlatform(): DesktopPlatform {
  const platform = process.platform;
  if (platform === 'darwin' || platform === 'win32' || platform === 'linux') return platform;
  // The release builds for exactly three platforms; anything else is not one this app was
  // shipped for, `platform` has no honest value to report.
  throw new Error(`Adminium Desktop does not support the "${platform}" platform.`);
}

/**
 * The config module is a set of pure functions over an explicit path, not a
 ...
 * before the server encrypts anything with it (a DSN saved by is unreadable if
 * the next boot generates a different secret), while `firstRun` stays a statement
 * about THIS boot's starting URL rather than a claim that the wizard was
 * finished. The wizard is gated server-side on "zero users", so a user who quits
 * halfway lands back in it next launch even though `config.json` now exists.
 */
  ...
  return {
    async ensureDataDir(dir) {
      try {
        ...
        // read-only directory succeeds — the failure this probe must catch is
        // a location the app cannot WRITE, and only writing proves that.
    ...
    async load() {
      ...
      // A config migrated forward is written back now that we know it loaded
      // cleanly — loadConfig deliberately leaves that to us.
...
/**
 * config.ts's {@link ConfigLogger} over main-process log.
 *
 * Without this the two call sites above default to config.ts's `silentLogger`,
 * and the step-3 plaintext-secret warning — the one that justifies
...
export function electronBootDeps(): DesktopBootDeps {
  ...
  const showLogs = async (): Promise<void> => {
    // "Help → 'Show logs' reveals the folder." `openPath` returns a NON-EMPTY
    // string on failure rather than throwing, which is a shape worth logging: a
    // bridge that reports success while nothing opened is worse than one that
    // reports the reason.
    const error = await shell.openPath(logsDir);
    if (error !== '') mainLog(`[main] could not open the logs folder: ${error}`);
  };

  // ── updater plumbing ── electron-updater is a CJS dependency; `require` it
  // lazily so a unit test that imports this module never loads it, and — the
  // correctness rule — so a `disabled` boot never even resolves it
  // (`createUpdateManager`'s `getUpdater` below is the only reader, and
  // `buildUpdateManager` skips it in disabled mode).
  const requireFromMain = createRequire(import.meta.url);
  /**
   * The ONE update-notification sink (`onUpdateEvent`), assigned when
   ...
   * `null` before then, and forever in `disabled` mode — which is exactly the
   * `UNAVAILABLE` the getter's consumers expect.
   */
  let updateManager: UpdateManager | null = null;
  ...
  return {
    ...
    // ── updater ── Delegates to `updates.ts`'s pure factory, adding the three
    // parts only this Electron side can: the env override (`process.env`), the
    // lazily-`require`d electron-updater, and the platform's self-replace
    // capability.
    createUpdateManager: ({ mode }) => {
      updateManager = buildUpdateManager({
        ...
        // even `require`d and never touches the network (correctness rule).
        ...
        getUpdater: (): UpdaterPort => {
          ...
          // (warns it checks on construction in some configs), so this runs only
          // for notify/manual, at most once — never in `disabled`, never in the
          // vitest program that imports this module.
        ...
        // The ONE notification pipeline. `emitUpdateEvent` is bound by
        // `registerBridge` (boot step 2), which always runs before this
        // (step 5).
    ...
    // The env kill-switch as a boot constant for the runtime snapshot. Reuses
    // `resolveUpdateMode`'s exact `ADMINIUM_DISABLE_UPDATES=1` rule (a
    ...
    createBackup: (wiring) =>
      createBackupCoordinator({
        ...
        // The "first idle moment after 03:00". `powerMonitor` is a main-process
    ...
    // ── native menu ── `isDev`: Toggle Developer Tools is allowed "in dev
    // builds only". `app.isPackaged` is Electron's own answer and cannot be
    // spoofed by an env var a user copied off a forum, which is what makes it
    // the right gate for a menu item that runs arbitrary script against the
    // loopback origin.
    installMenu: ({ handlers, t }) => {
      buildAppMenu({
        ...
        handlers,
        // The localized labels, resolved by the renderer and pushed over
    ...
    // The collision pre-flight, on a real socket. Not `probeBindable`
    ...
    registerBridge: (context) => {
      const handlers = registerIpcHandlers({
        ...
        ...context,
        /**
         * The "reachable URLs … for each non-internal interface, via
         * `os.networkInterfaces()`".
         *
         * THIS IS THE WIRING. `main/ipc.ts` has accepted this port since
         * and nothing ever passed it, so its `?? (() => [])` default answered
         * every call: `getRuntimeInfo().lanShare.urls` was `[]` in every build
         * that has ever run, and the panel describes had nothing to list.
        ...
        /**
         * A GETTER over the outer `updateManager`, which the step-5
         * `createUpdateManager` port assigns AFTER this bridge registers
         ...
         * `UNAVAILABLE` for, which also satisfies correctness rule: an
         * updater that is never constructed cannot make a network request.
         */
        updates: () => updateManager,
        // The capability plumbing. The host gates EVERY invoke on a fresh read
        // of the grant table over loopback — the same session-cookie transport
        // backup uses — and the escpos stub is v1's one provider. The origin is
        // derived from the LIVE runtime snapshot, not captured: the child's port
        ...
        // The About panel data.
        //
        // THIS IS THE WIRING: `main/ipc.ts` registered the two channels the moment
        // it was written, but they reject with INTERNAL "no port" until this
        ...
        // the main process can locate — to in-app viewers.
      ...
      // The ipc's typed emitter IS the ONE notification pipeline
