 ...
 * `main` / `preload` / `renderer`, but needs a fourth artifact — the
 ...
 *
 * The plan said "the server wrapper bundles `@adminium/server` and its workspace deps
 * with externalized native modules". We deliberately do NOT, because — the
 ...
 * with it (BYO round-trip is an acceptance criterion).
 *
 ...
 * Bundling bought nothing and cost correctness. Externalizing makes "all four
 * deployment modes run the identical `@adminium/server` process; only the
 * wrapper differs" literally true rather than approximately true — the desktop
 * app loads the same files `adminium start` does, resolved from
 * `apps/desktop/node_modules`, with their path arithmetic intact.
 *
 * `out/server/index.js` is therefore a few KB of wrapper. Packaging
 * `node_modules` into the asar (with `asarUnpack` for the `.node`
 * binaries).
 */
...
/**
 * The prod build "copies the prebuilt `@adminium/dashboard` `dist/` into
 * resources; the server serves it via its existing static handler".
 *
 * The dashboard is a BUILD OUTPUT here, never an import (desktop may use
 * "dashboard build output (static files)" and nothing else of it), so this
 * resolves the package's directory rather than importing it, and `desktop#build`
 * depends on `dashboard#build` in turbo.json so `dist/` exists. A miss is a hard
 * error: a desktop build that silently shipped no SPA would boot to a blank
 * window and pass every gate we have.
 */
 ...
 * exists, so their CSS cannot be fetched — it is copied next to them and
 * `<link>`ed relatively. `@adminium/tokens` is the source of truth for the
 * custom properties; pinning a copy of the two files here keeps the pages
 * honest without giving the shell an import edge into a package's internals.
 */
...
export default defineConfig({
  ...
  preload: {
    build: {
      // CJS, not ESM. The shell requires `sandbox: true`, and a sandboxed preload is
