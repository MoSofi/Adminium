  ...
  return invoicingManifest([
    ...
    // A table the install makes: the app may say what its own columns are.
    {
      ...
      columns: [
        ...
        // Who wrote the note, and — were it kept — their key, copied into a column anyone reads.
        { ref: 'user_id', type: 'fk', references: 'users', nullable: true },
        { ref: 'author_key', type: 'text', maxLength: 64, nullable: true, rules: { copy: { via: 'user_id', from: 'api_token' } } },
      ],
    },
  ]);
}
...
describe.each(LEGS)('an app that reuses a table with secrets — %s', (dialect, available) => {
  ...
  beforeAll(async () => {
    ...
    h = await installInvoicing(dialect, reusingManifest(), undefined, {}, {
      prepare: async (run) => {
        // A key that numbers itself already: the app's notes link to it.
        const key = dialect === 'postgres' ? 'integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY' : dialect === 'mysql' ? 'integer AUTO_INCREMENT PRIMARY KEY' : 'integer PRIMARY KEY';
        await run(`CREATE TABLE studio_users (id ${key}, email varchar(254) NOT NULL, api_token varchar(64), reset_token varchar(16))`);
  ...
  it.skipIf(!available)('copies none of them into a column of its own, and says so', async () => {
    const { targetOf } = await writerFor(h);
    const notes = targetOf('notes').table;
    const skipped = (h.reply['rules'] as { skipped: { table: string; column: string; op: string; reason: string }[] }).skipped;
    expect(skipped).toContainEqual({
      table: notes.id,
      column: 'author_key',
      op: 'column.copy',
      reason: `"${h.real('users')}.api_token" is a secret, so no column copies it unless it is one too.`,
    });
    expect((await overridesRepo(h.meta).listForConnection(h.connectionId)).filter((o) => o.op === 'column.copy')).toEqual([]);
  });
