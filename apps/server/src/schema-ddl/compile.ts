  ...
  switch (step.kind) {
    ...
    case 'add-column': {
      ...
      if (values !== undefined && values.length > 0) {
        statements.push(raw(checkStatement(t, column.name, values, dialect)));
      }
    ...
    case 'add-check': {
      ...
      return [raw(checkStatement(t, column, values, dialect))];
    }

    case 'drop-check': {
      const name = step.column ?? `ck_${bareName(step.table)}`;
      return [raw(`ALTER TABLE ${t} DROP CONSTRAINT ${quoteIdent(name, dialect)}`)];
    }
