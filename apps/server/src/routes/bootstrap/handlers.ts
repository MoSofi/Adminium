...
export async function bootstrapHandler(
  ...
  const [roles, prefs, pageRows, connectionRows, llmProvider, assistantName, placements, project, addOns] =
    ...
    /*
     * Add-ons ENABLED FOR THE DASHBOARD's own host key (51b). `dashboard` is
     * reserved in the manifest schema precisely because it is the host a stock
     * deployment attaches add-ons to, and `enabledForHost` is what already
     * answers "installed here, and switched on" — a disabled add-on keeps its
     * rows out of the rail without being uninstalled.
     */
    /*
