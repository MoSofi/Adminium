...
/**
 * Notification-centre routes (M7 reports/notifications track), mounted
 * under `/api/v1`. Everything here is per-user `/me` surface — RBAC needs
 * no new keys, the session IS the scope:
 *
 ...
 * likewise until a transport exists (explain, never hide).
 *
...
export const NOTIFICATION_EVENT_KEYS: readonly string[] = [
  ...
  // `email.campaign` is the opt-out for campaign mail (its `email` channel is
  // the whole point; the in-app half is inert), `email.campaign.sent` the
  // creator's notice when a run finishes.
  'email.campaign',
  'email.campaign.sent',
];

/**
 * Email delivery EXISTS now; what may be missing is a configured relay, so
 * this reason describes an unconfigured install rather than a build that
  ...
  return async (app) => {
    ...
    app.put(
      ...
      async (request) => {
        ...
        // transport fact reported alongside, never a write filter.
