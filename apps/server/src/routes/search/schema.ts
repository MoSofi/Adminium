...
/**
 * Zod schemas for `GET /api/v1/search`.
 *
 ...
 * count — "5+" semantics, never a full COUNT(*) (honest and fast). Empty
 * record groups are omitted unless the table timed out, in which case the
 * group ships `partial: true` with no hits rather than failing the request
 * (degradation rule).
 */

import { z } from 'zod';

/** Per-group hit cap. The ⌘K palette calls with `limit=3`. */
...
export const searchQuery = z.object({
  /** Needle; `ILIKE %q%` semantics, ≥ 2 chars after trimming. */
  q: z.string().min(2),
...
export const searchRecordHit = z.object({
  ...
  label: z.string(),
  /**
   * Row context, e.g. `unit_price 18 · status active` — up to two
   * leading readable columns beyond the label. Masked/secret columns are
   * excluded; omitted when the row has nothing else to show.
   */
...
export const searchRecordGroup = z.object({
  ...
  pageSlug: z.string(),
  count: z.number(),
  /** True when this table's search timed out — degraded, not failed. */
