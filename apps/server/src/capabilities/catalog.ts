...
/**
 * The host-capability vocabulary the desktop shell can be granted.
 *
 ...
 * concrete id, the ESC/POS receipt printer the POS micro-SaaS will use; the list
 * grows with the drivers.
 *
 ...
 * that ever sees a device (the server owns data, not hardware). The server owns
 * the id vocabulary — enough to validate a grant — and the grant table itself
 * (`adminium_settings`, reached over the loopback REST API by main).
 */

import { z } from 'zod';

/**
 * The dotted capability ids this build knows how to grant. Reverse-mapped
 * advisory `capabilities` vocabulary: a manifest declaring `receipt-printer`
 * needs `printer.escpos`.
 */
