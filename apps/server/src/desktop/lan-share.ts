...
/**
 * LAN share, as the SERVER can see it.
 *
 * The desktop shell owns the DECISION — `config.json`'s `lanShare` is main's
 * file, and main is what re-forks this process with `ADMINIUM_HOST=0.0.0.0` — so
 * nothing here toggles anything. What lives here are the two questions only a
 * process holding the meta store and its own listening socket can answer:
 *
 *  1. {@link lanShareActive} — am I bound wide, or to loopback? The runtime
 * chip needs this, forbids it the obvious source: "The chip is a `@adminium/ui`
 *     component fed by `GET /api/v1/system/info` + a lightweight
 ...
 * anyone but the super admin to connect? Both are meta-store reads, makes this
 *     process the only thing allowed to make them.
 *
 ...
 * The invariant is stated in the same terms ("The server binds 127.0.0.1
 ...
 *
 * `0.0.0.0` is what env block names and what the shell sends. `::` (and its
 * bracketed spelling) is the IPv6 wildcard, which reaches the same conclusion
 * by the same mechanism — and which a fleet admin or a hand-edited launch could
 * produce. Anything else — a loopback literal, a single NIC's address, a
 * hostname — is not a wildcard. Note that a specific LAN address like
 * `192.168.1.5` is deliberately NOT here: it is reachable, but the desktop
 * shell never produces it (flips between loopback and `0.0.0.0` and nothing
 * else), and treating an arbitrary literal as "sharing" would require this
 * module to enumerate interfaces — which is main's job (`os` lives there, and
 * this process may be in a container).
 */
...
/**
 * The toggle, as observed rather than as configured: is THIS process reachable
 ...
 * for a server behind a reverse proxy, and it is not "LAN share". The toggle is a
 ...
 * make chip meaningless on the one runtime that renders it and wrong on every
 * other.
 */
...
/** What share panel shows, minus the URLs (which only main can enumerate). */
export interface LanShareStatus {
  ...
  lanSessions: number;
  /**
   * Active users who are not super admins — precondition ("at least one
...
/**
 * The "connected-session count (from `adminium_sessions` with non-loopback
 * IPs)".
 *
...
/**
 * The precondition input: is there anybody to invite, or has the admin only
 * got themselves?
 *
 ...
 * therefore does NOT count: the precondition exists so the admin does not
 ...
 * "other user" and wave the precondition through.
 */
