...
/**
 * `core` plugin: cookie support (signed with ADMINIUM_SECRET), the same-origin
 * CORS default — no CORS headers are emitted unless ADMINIUM_CORS_ORIGINS opts
 * specific origins in — plus the -item-5 security headers (@fastify/helmet)
 * and the rate limiter (@fastify/rate-limit). Request-id, redaction, and the
 ...
 * this table stays the single owner of the limits ("Limits are constants in
 * `apps/server/src/plugins/core.ts` (`RATE_BUCKETS`)").
 *
 ...
 *
 * Rule 2 exists because the buckets asks for beyond the auth doors —
 ...
 * doing nothing wrong.
 *
 * Buckets are FIXED-WINDOW counters in one shared in-memory map ("the
 ...
 * guarantees a LAN peer hammering `/auth/desktop-session` can never lock the
 * loopback exchange out. Everything else keys by PRINCIPAL — api key, else
 * session user, else ip — because those routes are authenticated and one
 * shared office NAT should not be one budget.
 *
 ...
 *
 * NOT DONE, deliberately: the `adminium_settings.security` override mentions.
 * There is no `security.*` settings namespace — `PUT /settings/security` is a
 * route name whose fields map to `auth.*` — so an override would mean inventing
 * new registry keys, and `corePlugin` composes before any meta store exists and
 * has no meta handle to read them with. The buckets are the value; making them
 * editable is a separate wave with a settings surface behind it.
 *
 * A limit hit throws the SAME `RateLimitedError` handlers would throw, so the
 * 429 rides the one envelope (`details: { bucket, limit, resetAt }`) and the
 * plugin's own `retry-after` / `x-ratelimit-*` headers survive on the reply
 * the global error handler sends.
 *
 * ─── CSRF ─────────────────────────────────────────────────────────────────────
 *
 ...
 * theme pre-hydration flash guard, hashed from the build at boot so the
 * allowance tracks the artifact rather than a constant that can drift. No
 * `upgrade-insecure-requests` and no HSTS unless `ADMINIUM_TRUST_PROXY`
 * (behind Caddy/TLS): the desktop shell LAN share serve plain `http://`
 * loopback/LAN origins by design, and either directive would quietly break
 * them. `connect-src` adds `ws://<host> wss://<host>` derived from each
 * request's Host header, because CSP's `'self'` does not reliably cover
 * same-host WebSocket upgrades (`/ws`) across engines — but bare `ws:`/`wss:`
...
/** One bucket: a fixed window of `max` requests per `timeWindowMs`. */
export interface RateBucket {
  max: number;
  timeWindowMs: number;
  /**
   * What the counter is keyed on. `ip` for the unauthenticated doors (see the
   * module header — this is load-bearing for loopback); `principal` = api key
   * → session user → ip, in that order.
   */
  keyBy: 'ip' | 'principal';
}

/**
 * The bucket table. Keys are the `config.rateLimitBucket` markers routes
 * declare; a route declaring a bucket that is not in this table fails AT BOOT
 * (the onRoute hook throws), so the two spellings cannot drift apart
 * silently.
 *
 * - `auth-login` — `/auth/login`, `/auth/2fa/verify`, `/setup/super-admin`,
 *  `/auth/desktop-session` (5/min, row 1).
 * - `auth-password-forgot` — `/auth/password/forgot` (3/hour, row 2).
 * - `auth-password-reset` — `/auth/password/reset`: the token CONSUME side.
 *  Not in the table by name, but single-use + 30-min-TTL reset tokens are
 *  only as strong as the guess rate this cap sets.
 * - `search` — `GET /search` (60/min, row 5): each request fans out parallel
 *  ILIKE queries over up to 20 tables, the cheapest DoS lever of the
 *  unbucketed routes.
 *
 * - `api` — the row-1 general default, 300/min, applied to every other `/api/`
 *  route that does not match a narrower rule. Read the ip-fallback paragraph in
 *  the module header before changing the number.
 * - `widget-data` — `POST /widget-data/{query,batch}` (120/min): a dashboard
 *  paints many widgets at once, so this sits well above the general
 *  per-minute feel while still bounding a scripted fan-out.
 * - `data-io` — export/import CREATION (10/hour): `POST /exports` and `POST
 *  /imports`. Each one leads to a job that reads or writes a whole table. `run`
 *  is deliberately not charged — see {@link AUTO_BUCKETS}.
 * - `llm` — the provider-calling LLM routes (20/hour). Per principal, so it is
 *  20 assists per person per hour, not per install. `apply`/`undo`/`diff` are
 *  NOT in it: they replay a run that was already paid for.
 * - `file-bytes` — the row (30/hour). There is no `files` route group, so the
 *  budget lands on the four surfaces that actually move file bytes: `POST
 *  /branding/logo`, `POST /imports/upload`, `GET /imports/:id/error-report` and
 *  `GET /exports/:id/download`.
 * - `app-shell` — the SPA's COLD-START payload (900/min): the six routes a
 *  browser must GET to render the shell at all, before the user has asked for
 *  anything — nine requests, since the i18n bundle is fetched per namespace. Not
 *  a row; see the deviation note below for why it had to exist.
 *
 * DELIBERATE DEVIATIONS (documented here so the table can't drift
 * silently):
 ...
 * - `app-shell` is an EXTRA bucket the plan does not list. Row 1 reads as a budget
 *  for what a USER DOES, but the general bucket as built also charges what the
 *  APP COSTS TO BOOT, and those are different sizes. Every cold document load
 *  spends nine `/api/` GETs before the user has asked for anything:
 *  `/bootstrap`, `/branding`, `/system/info`, `/i18n/manifest`, three
 *  `/i18n/bundle/:locale/:namespace` namespaces and `/me/notifications`. At
 *  300/min that leaves one principal ~30 page loads a minute, floor to ceiling
 ...
 *  the cause: a 429 on `/bootstrap` or on a bundle does not degrade a feature,
 *  it replaces the entire app with the rate-limited state page.
 *
 ...
 * - The `adminium_settings.security` override mentions is not built — see the
 *  module header for why (there is no `security.*` namespace to read).
 */
export const RATE_BUCKETS = {
   ...
   * `file-bytes` hour on two records, original 30/hour figure predates the
   * batch flow it would refuse.
   *
...
const AUTO_BUCKETS: readonly {
  ...
  // Anchored on `/files` exactly, so `/files/:id/attach` and the rest of the
  // group fall through to `api` — they move no bytes.
...
/**
 * The in-memory store, as a @fastify/rate-limit custom-store class whose
...
export const corePlugin = fp<CorePluginOptions>(
  async (app, { env, staticRoot }) => {
    ...
    // Helmet's hook is global, so API replies, static assets and the SPA
    // fallback all carry the same headers.
    await app.register(fastifyHelmet, {
      contentSecurityPolicy: {
        ...
        directives: {
          ...
          'object-src': ["'none'"],
          /*
           * SAME-ORIGIN framing is permitted; everything else is
           * not.
           *
    ...
    app.addHook('onRoute', (route) => {
      ...
      if (spec === undefined) {
        ...
        throw new Error(
          `route ${methods} ${route.url} declares unknown rate-limit bucket "${bucket}" — add it to RATE_BUCKETS (plugins/core.ts)`,
        );
      }
      route.config = {
        ...route.config,
    ...
    // `preValidation`, not `onRequest` — see the module header.
    const csrf: CsrfDeps = {
    ...
    await app.register(fastifyRateLimit, {
      ...
      // Thrown by the plugin, serialized by the global error handler; the
