...
export function renderReadme(manifest: BundleManifest): string {
  ...
  return [
    ...
    '## Restoring it',
    '',
    'From a built source checkout (see docs: Quickstart from source), with',
    '`adminium` aliased to `node apps/server/dist/cli/index.js`:',
    '',
    '```sh',
    'export ADMINIUM_SECRET=…       # 32+ chars; the target instance\'s secret',
    'adminium import-zip --in <this-archive>.zip',
    'adminium start',
    '```',
    '',
