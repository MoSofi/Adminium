...
const made = { email: [] as string[], invoices: [] as string[], reports: [] as string[] };

test.beforeAll(async ({ browser }) => {
  const page = await browser.newPage();
  await signIn(page);
  await configureFakeProvider(page);
  // The invoice scenario builds FROM a template, and `basedOn` must name a
  // real one — the page's own acceptor refuses an id it cannot find, which is
  // the behaviour, not a gap to script around. A seeded install has none, so
  // this spec makes one.
  const created = await page.request.post('/api/v1/invoices', { data: { kind: 'template', starter: 'standard' } });
  expect(created.status(), 'seeding an invoice template').toBe(201);
  made.invoices.push(((await created.json()) as { id: string }).id);
  await page.close();
});
...
test.afterAll(async ({ browser }) => {
  ...
  for (const id of made.invoices) await page.request.delete(`/api/v1/invoices/${id}`);
...
test.describe('the page assistant', () => {
  ...
  test('(f) invoice manager → the ask → picks → result → create draft invoice', async ({ page }) => {
    await signIn(page);
    await page.goto('/invoices');
    await expect(page.getByTestId('invoices-toolbar')).toBeVisible();

    await openAssistant(page);
    await modal(page).getByTestId('assistant-chip').filter({ hasText: /invoice for a customer for last month/i }).first().click();

    // A question, not a draft: the model does not guess which template to use.
    const ask = modal(page).getByTestId('assistant-ask');
    await expect(ask).toBeVisible({ timeout: 30_000 });
    await expect(ask.getByTestId('assistant-ask-go')).toBeDisabled();
    await ask.getByTestId('assistant-pick').first().click();
    await expect(ask.getByTestId('assistant-ask-go')).toBeEnabled();
    await ask.getByTestId('assistant-ask-go').click();

    await expect(modal(page).getByTestId('assistant-steps')).toHaveAttribute('data-state', /done|failed/, { timeout: 30_000 });
    const result = modal(page).getByTestId('assistant-result');
    await expect(result).toBeVisible();
    // A record has no sheet: the field grid, priced by the page's money law.
    await expect(result.getByTestId('assistant-draft-total')).toBeVisible();
    ...
    await expect.poll(async () => {
      const reply = await page.request.get('/api/v1/invoices?kind=invoice');
      const body = (await reply.json()) as { items: { id: string; name: string; status: string }[] };
      const row = body.items.find((item) => item.name === 'Contoso Ltd — March');
      if (row !== undefined && !made.invoices.includes(row.id)) made.invoices.push(row.id);
      return row?.status;
