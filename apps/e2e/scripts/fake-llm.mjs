...
export function createFakeLlmServer() {
  return createServer((req, res) => {
    ...
    // The unmatched request line goes to STDERR, not into the reply. Echoing
    // it back reflects user-controlled text into a response body, which is the
    // shape of a cross-site scripting bug even on a fixture nothing but the
    // e2e run can reach — and the log is where a mis-routed scenario is
    // actually read from anyway.
    process.stderr.write(`fake-llm: no route for ${req.method ?? ''} ${url}\n`);
    res.statusCode = 404;
    res.setHeader('content-type', 'application/json');
