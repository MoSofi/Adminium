...
description: An assistant that knows the page you opened it from — it drafts emails and reports in that page's own format, and it never saves anything itself.
---

Open an Adminium page that builds documents — Email templates, Report
builder — and there is an **Ask** button in the header. It opens an assistant that already knows what that page holds: the
...
- **It never publishes.** A saved draft is a draft: an email template arrives
  disabled, a report arrives with status `draft`.
- **It never changes existing rows.** A draft changes no customer record, and
  the window says so.
