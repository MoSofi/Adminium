...
/**
 * The capability vocabulary and status logic, as the SPA sees it. Pure — no
 * React, no bridge — so the one contract that MUST hold on every runtime is
 * testable without a DOM: "non-desktop runtimes report every capability
 * `unavailable`".
 *
 ...
 * (SDK surface), never error.
 */

/** `GET /api/v1/system/info`'s `runtime`. */
export type Runtime = 'selfhost' | 'desktop' | 'cloud';

/** The frozen descriptor status. */
export type CapabilityStatus = 'available' | 'unavailable' | 'stub';

/** The frozen descriptor, restated for the SPA (mirrors
 * `@adminium/desktop/api`). */
 ...
 *
 * v1 ships one: the ESC/POS receipt printer the POS micro-SaaS needs. Kept in
 * lockstep with the server's `KNOWN_CAPABILITY_IDS` and the main process's
 * providers; `model.test.ts` pins the id set so a rename here is a red test, not
 * a silent divergence from the driver.
 */
export interface CapabilityMeta {
  ...
  nameKey: string;
  nameDefault: string;
  /** The consent scope line, e.g. "Print to receipt printers" (copy). */
  scopeKey: string;
  scopeDefault: string;
 ...
 * every catalogued capability is `unavailable`. This is cross-runtime rule,
 *    made concrete in the one place that runs on every runtime.
 */
