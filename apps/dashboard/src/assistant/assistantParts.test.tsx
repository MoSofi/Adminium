 ...
 */
import { renderHook, act, cleanup, render, screen, within } from '@testing-library/react';
...
describe('the record preview', () => {
  it('lists the lines and says what is not touched', async () => {
    const { DraftPreview } = await import('./parts/DraftPreview.js');
    render(
      <DraftPreview
        basedOnLabel="Consulting"
        amounts={['$200.00']}
        total="$200.00"
        artefact={{
          body: {
            ...
            issued: '2026-03-01',
            items: [{ desc: 'Work', from: 'time_entries', qty: '2', rate: '100.00' }],
          },
        }}
      />,
    );
    const grid = screen.getByText('Account').closest('dl');
    expect(within(grid as HTMLElement).getByText('Company 1')).toBeTruthy();
    expect(screen.getByText('time_entries')).toBeTruthy();
    expect(screen.getByText('1 line')).toBeTruthy();
    // The money is the PAGE's, handed in already formatted — this component
    // computes none of it, and shows the model's arithmetic nowhere.
    expect(screen.getByTestId('assistant-draft-total').textContent).toBe('$200.00');
    expect(screen.getByText('No customer rows are changed, and no email is sent.')).toBeTruthy();
  });
});
