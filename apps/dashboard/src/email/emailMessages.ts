 ...
 * campaigns.md §6.1; `DEFERRED_NAMESPACES`): the manager and the editor
 * render only after it is in the store, so an `email:` key never falls back
 * to English for a translated locale. The two routes `use()` this promise
