 ...
 * without typing costs nothing) and immune to a later in-place mutation of
 * the object that was snapshotted.
 *
 * Pure: every function returns a new history; the hook that owns the draft
 * decides when to push (the comp pushes on focus — one step per editing
 * session of a field — and before every structural change).
 */
...
export function undoHistory<T>(history: History, current: T): [History, T] | null {
  ...
  return [
    { past: history.past.slice(0, -1), future: [...history.future, snapshot(current)] },
    JSON.parse(snap) as T,
  ];
}
...
export function redoHistory<T>(history: History, current: T): [History, T] | null {
  ...
  if (snap === undefined) return null;
  return [
    { past: [...history.past, snapshot(current)], future: history.future.slice(0, -1) },
    JSON.parse(snap) as T,
  ];
}
