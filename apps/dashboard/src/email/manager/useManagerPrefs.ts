 ...
 * in `localStorage` so they survive a reload (39-email-templates-and-
 * campaigns.md Appendix A §M2). They are conveniences of one browser, not
 * workspace state: nothing about a template changes when someone prefers the
 * list, so they never touch the server or `adminium_user_prefs`.
 *
 ...
 * with the defaults rather than not at all (the `palette/recent.ts` rule).
 */
...
export const MANAGER_PREFS_KEY = 'adminium-email-manager';

/** The comp's initial state (782, 789): gallery, ungrouped. */
