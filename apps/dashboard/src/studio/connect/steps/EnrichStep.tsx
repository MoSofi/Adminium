...
export function EnrichStep({ state, onPatch, onOpenReview, pollIntervalMs }: EnrichStepProps) {
  ...
  const intent = state.enrichIntent;
  const sections = state.enrichSections;
  const locales = state.enrichLocales;
  const sampling = state.enrichSampling;

  const providerAvailable = providerCardEnabled({ providerConfigured, connectionId, sourceIsFile });
    ...
    <section aria-label={t('studio.enrich.title', 'Enrich with AI')} className="flex flex-col gap-4">
      ...
      >
        <RadioCard
          ...
          description={
            providerAvailable
              ? t('studio.enrich.provider.description', 'Run enrichment now against your configured provider. You review every suggestion as a diff.')
              : t('studio.enrich.provider.unconfigured', 'No AI provider is configured yet — copy a prompt to your own tool below, or configure a provider first.')
          }
          icon={<Sparkles />}
        />
        <RadioCard
          value="byo"
          title={t('studio.enrich.byo.cardTitle', 'Copy a prompt to my own AI tool')}
          description={t(
