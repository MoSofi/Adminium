...
/**
 * The built-in page templates' loaders, for `resolvePageTemplate` in
 * `templates.tsx`. Each template is wrapped in the project scope
 * (`project/scope.tsx`): the cells, cards and toasts of a project's own
 * code, which is how tables and dashboards find them.
 *
 ...
 * growth to the en-US i18n catalogue and naming a namespace split as the fix.
 * Deleting the entire catalogue and rebuilding measures 48.7 KiB gz — real,
...
const builtinTemplates: Record<string, TemplateLoader> = {
  ...
  // The record detail page — resolved by TemplateMount from the
  // envelope's `config.detail.template` when the `/r/$recordId` child
  // route is active.
  ...
  // A page written by hand in the project folder, `pages/<slug>.tsx`. The
  // server writes these rows; Studio cannot create one.
