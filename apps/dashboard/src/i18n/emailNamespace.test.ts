 ...
 * 3. Nothing outside `src/email` may read an `email:` key: the surface that
...
const PAIRED = /'email:([A-Za-z0-9_.-]+)'\s*,\s*(?:[A-Za-z][A-Za-z0-9_]*\s*:\s*)?('(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*")/g;
const ANY_KEY = /'email:([A-Za-z0-9_.-]+)'/g;
...
function emailSites(): { sites: Site[]; unpaired: string[] } {
  const sites: Site[] = [];
  const unpaired: string[] = [];
  for (const file of sourceFiles(EMAIL, false)) {
    ...
    for (const match of src.matchAll(ANY_KEY)) {
      ...
      unpaired.push(`${file}: email:${match[1] ?? ''}`);
    }
  }
  return { sites, unpaired };
}

describe('the deferred `email` namespace', () => {
  const { sites, unpaired } = emailSites();

  it('the scan finds the email surface (regex/tree sanity)', () => {
    expect(new Set(sites.map((s) => s.key)).size).toBeGreaterThan(150);
  });
  ...
  it('is read from nowhere but the email surface', () => {
    ...
    for (const file of sourceFiles(SRC, true)) {
      if (file.startsWith(`${EMAIL}/`) || file === EMAIL) continue;
      if (file === join(SRC, 'i18n', 'emailNamespace.test.ts')) continue;
      const src = readFileSync(file, 'utf8');
      for (const match of src.matchAll(ANY_KEY)) outside.push(`${file}: email:${match[1] ?? ''}`);
    }
    expect(
      outside,
      `a deferred namespace read outside the surface that loads it — these render English until\n` +
        `somebody opens the email templates, and never in the user's own language:\n${outside.join('\n')}`,
    ).toEqual([]);
  });
});
