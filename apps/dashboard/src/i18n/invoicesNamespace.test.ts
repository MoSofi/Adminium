 ...
 * 3. Nothing outside `src/invoices` may read an `invoices:` key: the surface that
...
const INVOICES = join(SRC, 'invoices');

const PAIRED = /'invoices:([A-Za-z0-9_.-]+)'\s*,\s*(?:[A-Za-z][A-Za-z0-9_]*\s*:\s*)?('(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*")/g;
const ANY_KEY = /'invoices:([A-Za-z0-9_.-]+)'/g;
...
function catalogued(key: string): string | null {
  let node: ResourceBundle[string] | undefined = EN_US_RESOURCES.invoices;
...
function invoiceSites(): { sites: Site[]; unpaired: string[] } {
  const sites: Site[] = [];
  const unpaired: string[] = [];
  for (const file of sourceFiles(INVOICES, false)) {
    ...
    for (const match of src.matchAll(ANY_KEY)) {
      ...
      unpaired.push(`${file}: invoices:${match[1] ?? ''}`);
    }
  }
  return { sites, unpaired };
}

describe('the deferred `invoices` namespace', () => {
  const { sites, unpaired } = invoiceSites();

  it('the scan finds the invoices surface (regex/tree sanity)', () => {
    expect(new Set(sites.map((s) => s.key)).size).toBeGreaterThan(400);
  });
  ...
  it('every key resolves in the en-US invoices bundle', () => {
    const missing = [...new Set(sites.filter((s) => catalogued(s.key) === null).map((s) => s.key))].sort();
    expect(missing, `keys missing from packages/i18n/locales/en-US/invoices.json:\n${missing.join('\n')}`).toEqual([]);
  });
  ...
  it('is read from nowhere but the invoices surface', () => {
    const outside: string[] = [];
    for (const file of sourceFiles(SRC, true)) {
      if (file.startsWith(`${INVOICES}/`) || file === INVOICES) continue;
      if (file === join(SRC, 'i18n', 'invoicesNamespace.test.ts')) continue;
      const src = readFileSync(file, 'utf8');
      for (const match of src.matchAll(ANY_KEY)) outside.push(`${file}: invoices:${match[1] ?? ''}`);
    }
    expect(
      outside,
      `a deferred namespace read outside the surface that loads it — these render English until\n` +
        `somebody opens the invoices, and never in the user's own language:\n${outside.join('\n')}`,
    ).toEqual([]);
  });
});
