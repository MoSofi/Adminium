...
/**
 * The deferred `dataio` namespace's contract (`DEFERRED_NAMESPACES`; the
 * `studio` gate's twin, `studioNamespace.test.ts`):
 *
 * 1. Every `dataio:` key must exist in the en-US bundle.
 ...
 * 3. Nothing outside `src/data-io` may read a `dataio:` key: the surface that
 *    loads the namespace is the only one that may depend on it.
 *
 * Rule 3 has real teeth here. `data-io/routes.tsx` is imported STATICALLY by
 * the router, so it is in the entry chunk and can never await this namespace —
 * it titles its pages from `common:nav.imports`/`nav.exports` instead. If a
 * later edit points it back at a `dataio:` key, this test is what says so.
 */
...
const DATAIO = join(SRC, 'data-io');

const PAIRED = /'dataio:([A-Za-z0-9_.-]+)'\s*,\s*(?:[A-Za-z][A-Za-z0-9_]*\s*:\s*)?('(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*")/g;
const ANY_KEY = /'dataio:([A-Za-z0-9_.-]+)'/g;
...
function catalogued(key: string): string | null {
  let node: ResourceBundle[string] | undefined = EN_US_RESOURCES.dataio;
...
function dataioSites(): { sites: Site[]; unpaired: string[] } {
  const sites: Site[] = [];
  const unpaired: string[] = [];
  for (const file of sourceFiles(DATAIO, false)) {
    ...
    for (const match of src.matchAll(ANY_KEY)) {
      ...
      unpaired.push(`${file}: dataio:${match[1] ?? ''}`);
    }
  }
  return { sites, unpaired };
}

describe('the deferred `dataio` namespace', () => {
  const { sites, unpaired } = dataioSites();

  it('the scan finds the dataio surface (regex/tree sanity)', () => {
    expect(new Set(sites.map((s) => s.key)).size).toBeGreaterThan(190);
  });
  ...
  it('every key resolves in the en-US email bundle', () => {
    ...
    expect(missing, `keys missing from packages/i18n/locales/en-US/dataio.json:\n${missing.join('\n')}`).toEqual([]);
  });
  ...
  it('is read from nowhere but the email surface', () => {
    ...
    for (const file of sourceFiles(SRC, true)) {
      if (file.startsWith(`${DATAIO}/`) || file === DATAIO) continue;
      if (file === join(SRC, 'i18n', 'dataioNamespace.test.ts')) continue;
      const src = readFileSync(file, 'utf8');
      for (const match of src.matchAll(ANY_KEY)) outside.push(`${file}: dataio:${match[1] ?? ''}`);
    }
    expect(
      outside,
      `a deferred namespace read outside the surface that loads it — these render English until\n` +
        `somebody opens the dataio surface, and never in the user's own language:\n${outside.join('\n')}`,
    ).toEqual([]);
  });
});
