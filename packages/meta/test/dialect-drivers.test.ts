...
/**
 * Regression guard for the "matrix quietly only tests sqlite" trap: the
 * dialect harness (test/helpers/db.ts) gates the Postgres and MySQL legs on
 * `Boolean(TEST_*_URL) && resolvable(driver)`. If `pg` / `mysql2` are not
 * dependencies of @adminium/meta, `resolvable()` is always false and those
 * legs are skipped even when their URL is set — so viewsRepo's cross-dialect
 * SQL ships to Postgres/MySQL untested.
 *
