...
/**
 * `adminium_schema_changes.acknowledged_rows`.
 *
 * The migration spec listed this column and `0023_schema_authoring` did not
 * ship it, so the acknowledgement — the operator confirming they saw the row
 * count before a rewrite ran — was collected on the wire and then forgotten.
 * The ledger is the record of what an operator authorised; "they were shown 1.4
 * million rows and said yes" is exactly the kind of fact it exists to hold, and
 * the kind nobody can reconstruct afterwards.
 *
