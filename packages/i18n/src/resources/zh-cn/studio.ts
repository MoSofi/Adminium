...
export default {
  "addOns": {
    "browse": {
      ...
      "install": "Install",
      "missing": "缺失",
      "missingBody": "它的文件不在此服务器上，因此其中任何部分都不会加载。",
    ...
    "installed": {
      ...
      "missing": "缺失",
      "missingBody": "它的文件不在此服务器上，因此其中任何部分都不会加载。请重新安装，或将其移除。",
  ...
  "documents": {
    ...
    "deleteConfirm": {
      "title": "删除此映射？",
      "body": "触发它的规则也会一并删除，已据此生成的文档将失去回链。此操作无法撤销。",
      "prompt": "输入 {name} 以确认",
      "confirm": "删除映射"
    },
    "deleteFailed": "未删除该映射",
  ...
  "enrich": {
    ...
    "noTablesBody": "此数据库还没有任何表，因此 AI 没有可标注或分组的内容。请继续——表创建后，您可以随时在“设置 → AI”中运行 AI 增强。",
    "noTablesTitle": "没有可增强的表",
    "generatePrompt": "生成提示",
    "intentLabel": "你希望如何丰富？",
    "localeLocked": "（必填）",
  ...
  "hostedApps": {
    ...
    "domains": {
      ...
      "savedTitle": "已保存",
      "stepDns": "在你的 DNS 中把该主机指向这台服务器——记录类型和目标与你访问本控制台所用的地址相同。",
      "stepProxy": "在你的反向代理上为该主机添加独立的站点配置块，并原样透传 Host 请求头——然后重新加载代理。只改配置文件不会影响已在运行的进程。",
      "stepSignIn": "员工界面会要求你重新登录：会话 Cookie 只属于一个主机，因此映射的主机会先把你带到它自己的登录页。",
    ...
    "installed": {
      ...
      "cannotUpdate": "v{version} 无法直接更新此版本。请先卸载它，再安装 v{version}。",
      "missing": "缺失",
    ...
    "names": {
      "label": "{app} 的名称",
      "save": "保存名称",
      "subtitle": "每个应用的名称——既用于它自己的界面，也用于本控制台的侧边栏。留空则使用应用构建时自带的名称。",
      "title": "应用名称"
    },
    ...
    "columns": {
      "title": "将 {app} 更新到 v{version}",
      "subtitle": "此版本需要数据库中某些表尚未包含的列。",
      "body": "Adminium 可以为您添加这些列。执行任何操作前您都会看到确切的语句，不会删除任何内容，且只有在列创建后才会更新应用。",
      "alsoCreates": "此更新还会创建以下表：",
      "noDdl": "Adminium 无法在此添加这些列",
      "failed": "无法添加这些列",
  ...
  "hub": {
    ...
    "card": {
      ...
      "timezoneNone": "未设置——日期以 {zone} 显示，即此服务器的时区"
    },
    "connectNew": "新建连接",
    "delete": {
  ...
  "pages": {
    ...
    "field": {
      ...
      "table": "数据表",
      "tableChoose": "选择数据表…",
      "tableCreateHint": "此页面读取的数据表。",
      "tableNeedsConnection": "请先选择数据源。",
      "tableNoConnection": "请先连接数据库，此页面由其中一个数据表构建。",
      "tableNone": "未绑定",
      "template": "模板",
    ...
    "fit": {
      "alternatives": {
        "title": "使用一个已经适用的表",
        "help": "不会向你的数据库写入任何内容——只是让该页面指向一个已具备所需内容的表。",
        "use": "使用此表"
      },
      "checkFailed": "Adminium 无法检查此表",
      "checkFailedBody": "你仍然可以创建该页面。如果此表不适用，创建时会告知你。",
      "columns": {
        "title": "为此表补上缺少的内容",
        "help": "Adminium 会向你的表中添加这些列。在执行之前你会看到确切的语句，而且不会删除任何内容。",
        "review": "查看更改",
        "confirm": "执行",
        "failed": "操作未成功",
        "partial": "此页面所需的其中一项无法为你添加，因此之后仍会不完整。",
        "cannot": "这一项 Adminium 无法为你添加",
        "cannotBody": "此页面需要指向另一个表的关联，需要在 Studio → 结构 中设置。",
        "halfDone": "列已添加，但 Adminium 无法记录它们的含义",
        "halfDoneBody": "无需重新执行——这些列已存在。请在 Studio → 结构 中设置它们的含义，或请管理员代为设置。"
      },
      "needs": "要构建此页面，该表还需要：",
      "noDdl": "Adminium 无法为你修改此表",
      "role": {
        "eventDate": "每行一个日期",
        "title": "一个用作每行标题的文本列",
        "status": "一个状态列，其取值表示工作流程阶段",
        "personFk": "指向人员表的关联",
        "shiftType": "一个说明每行属于哪种班次的列"
      },
      "slotOnly": "此表中没有内容可以填充“{slot}”区域。",
      "tag": {
        "title": "使用你已有的列",
        "help": "这只记录该列的含义。你的数据库不会被修改，并且可以在 Studio → 结构 中撤销。",
        "action": "使用此列",
        "failed": "无法标记该列"
      },
      "title": "此表暂时无法支撑该页面",
      "table": {
        "title": "为此页面新建一个表",
        "help": "Adminium 会创建一个包含此页面所需全部内容的表。执行任何操作前，您都会看到确切的语句，且不会改动您的其他表。",
        "open": "或为此页面新建一个表",
        "name": "表名",
        "nameTaken": "已存在同名的表。",
        "nameInvalid": "请使用小写字母、数字和下划线，并以字母开头。",
        "people": "将每一行分配给以下表中的人员",
        "peopleNew": "一个新的人员表",
        "peopleCreated": "还会创建“{table}”，一个记录行所分配人员的小表。",
        "noCompose": "使用此名称的表无法用于此页面。请换一个名称。",
        "confirm": "创建表",
        "noDdl": "Adminium 无法在此创建表",
        "halfDone": "表已创建，但 Adminium 无法记录其列的含义",
        "halfDoneBody": "无需再次运行——表已存在。请在 Studio → 架构 中设置其列的含义，或请管理员设置。",
        "notReread": "表已创建，但 Adminium 暂时无法重新读取它",
        "notRereadBody": "无需再次运行。请在 Studio → 数据连接 中刷新架构，然后在此选择新表。"
      },
      "related": {
        "title": "使用关联表的日期",
        "help": "不会向您的数据库写入任何内容。页面基于与此表关联的表构建，每个条目显示此表中的名称。",
        "reason": "日期来自“{date}”，每项以“{title}”为标题（通过“{via}”）",
        "chosen": "基于 {table} 构建，每个条目以 {from} 的“{title}”为标题",
        "undo": "返回 {table}"
      }
    },
    "form": {
  ...
  "remap": {
    ...
    "rules": {
      ...
      "decided": {
        ...
        "stampCreate": "创建该行时设为{what}",
        "stampChange": "当 {column} 变为 {values} 时设为{what}",
        "stampNow": "当前时间",
        ...
        "stampByOriginOwn": "公开端为“{public}”，员工端为员工所选",
        "stampCopy": "{column} 的值",
        "normalizeEmail": "去除首尾空格并以小写保存",
        "normalizeTrim": "去除首尾空格后保存",
        "states": "只能按其规则允许的步骤变更",
        "statesLock": "当其为 {states} 时该行被锁定"
      },
  ...
  "settingsAi": {
    "assistant": {
      ...
      "rowData": {
        "label": "允许 {name} 读取数据表行",
        "hint": "开启后，{name} 可以把你的角色有权读取的数据行发送给已配置的服务商 — 已脱敏、每次请求最多 50 行，并列在“已读取的来源”中。关闭后，它只依据文档和架构工作。"
      },
      "save": "保存",
      "saveFailed": "无法保存助手设置。请重试。",
      "saved": "助手设置已保存",
      "subtitle": "它在这里的名称，以及它可以读取的内容。",
      "title": "助手"
    },
    "byo": {
  ...
  "tables": {
    "emptyBody": "此数据库还没有任何表。您仍然可以继续——创建表后，在此连接上使用“重新内省”即可将其导入。",
    "emptyFileBody": "此架构文件未定义任何表。请返回并上传其他文件，或直接继续。",
  ...
  "wizard": {
    ...
    "startOver": {
      "action": "重新开始",
      "body": "这里输入的所有内容都会被清除，向导将回到第一步。",
      "bodyCreated": "这里输入的所有内容都会被清除，向导将回到第一步。Adminium 已创建的连接不会被删除——它仍保留在“数据连接”中。",
      "confirm": "重新开始",
      "keep": "继续",
      "title": "重新开始此向导？"
    },
  ...
  "appPublicAccess": {
    ...
    "warning": {
      ...
      "noEmailSignIn": "尚未设置电子邮件，因此无法向任何人发送登录链接。",
      "noPublicAddress": "此应用没有公开地址，因此无法发送登录链接。请将域名映射到其客户界面，或设置服务器的公开地址。"
    },
    "createConfirmed": "添加到 {table}，并收到确认邮件"
  },
  "addOnNeeded": {
    "appDisabled": "已关闭——仍然需要它",
    "appInstalling": "安装尚未完成——仍然需要它",
    "close": "关闭",
    "confirm": {
      "switchOff": "仍然关闭",
      "uninstall": "仍然卸载"
    ...
    "lead": {
      "feature": "{app} 的 {features} 将关闭。",
      "switchOff": "无法为 {app} 关闭 {addOn}。{app} 需要它。",
      "uninstall": "无法卸载 {addOn}。{count, plural, other {{apps} 需要它。}}"
    },
    "note": {
      "feature": "{apps} 的其余部分无需它也能运行。",
      "switchOff": "要关闭它，请先卸载 {app}。",
      "uninstall": "要卸载它，请先卸载 {apps}。"
    },
    "openApp": "打开 {app}",
    "subtitle": "v{version}",
    ...
    "title": {
      "switchOff": "为 {app} 关闭",
      "switchOffAsk": "为 {app} 关闭？",
      "uninstall": "卸载 {addOn}",
      "uninstallAsk": "卸载 {addOn}？"
    },
    "useFeature": "{app}（用于：{features}）",
    "useRequired": "{app}（必需）",
    "useSuggested": "{app}（建议）",
    "usedBy": "使用者",
    "usedByLine": "使用者：{apps}"
  ...
  "appAddOns": {
    ...
    "block": {
      "download": "请先下载 {addOn}：它在附加组件目录中，尚未在此服务器上。",
      "noVersion": "{app} 需要 {addOn} {version}，但这里没有这样的版本。",
      "problem": "无法随 {app} 安装 {addOn}。原因见其所在行。",
      "tooOld": "{app} 需要 {addOn} {version}。",
      "unavailable": "{app} 需要 {addOn}，但这里无法获得。",
      "untick": "这里无法将 {addOn} 与 {app} 一起使用。取消勾选即可在不带它的情况下安装 {app}。"
    },
    "card": {
      "connect": "连接",
      "connected": "已将 {addOn} 连接到 {app}",
      "consentConnect": "它将连接到 {app}。",
      "featureOff": "{features} 已关闭：在安装并连接 {addOn} 之前，其页面不会出现在侧边栏中。",
      "install": "安装",
      "installed": "已安装 {addOn} 并连接到 {app}",
      "metaAbsent": "未安装 · v{version} · {source}",
      "metaInstalled": "已安装 · v{version} · {source}",
      "metaOld": "已安装 · v{version}",
      "metaUnavailable": "未安装 · {source}",
      "notConnected": "未连接到 {app}",
      "openSettings": "打开其设置"
    },
    "done": {
      "installed": "{count, plural, other {同时安装了：{names}。设置位于 {addOns} 中。}}",
      "updated": "{count, plural, other {同时更新了：{names}。设置位于 {addOns} 中。}}"
    },
    "download": "下载",
    "downloading": "正在下载… {pct}%",
    "grant": "{roles} 将能够更改其设置，这些设置由它服务的所有应用共享。",
    "howTo": "如何添加附加组件",
    "intro": "{count, plural, other {{app} 可配合以下附加组件使用。}}",
    "meta": "v{version} · {source}",
    "needsFloor": "{app} 需要 {version} 或更高版本",
    "needsRange": "{app} 需要 {range}",
    "orLater": "{version} 或更高版本",
    "pill": {
      "feature": "用于：{features}",
      "required": "必需",
      "suggested": "建议"
    ...
    "plan": {
      "creates": "{count, plural, other {将在 {connection} 中创建 {count} 张表：{tables}}}"
    },
    "running": {
      "attach": "正在连接 {addOn}",
      "connected": "已连接",
    ...
    "shared": "附加组件是共享的。你安装的其他应用也可以使用它们。",
    "source": {
      "bundled": "随 Adminium 附带",
      "catalog": "来自附加组件目录",
      "none": "此 Adminium 未附带，附加组件目录中也没有可用的版本",
      "off": "此 Adminium 未附带，且附加组件目录已关闭",
      "upload": "已上传到此 Adminium"
    },
    "status": {
      "attached": "已连接到 {app}",
      "downloadFirst": "从附加组件目录下载后即会安装",
      "tooOld": "已安装 v{installed}——{need}",
      "unavailable": "{addOn} 在此 Adminium 上不可用",
      "willConnect": "已安装 · v{version} · 将连接到 {app}",
      "willInstall": "将被安装",
      "wontConnect": "已安装 · v{version} · 不会连接"
    },
    "stopped": {
      "atAddOns": "安装所需的附加组件失败，因此之后的步骤均未运行。"
    },
    "title": "附加组件",
    "uninstall": {
      "kept": "{addOn} 将保持安装。如果没有其他内容使用它，可在“附加组件”中卸载。",
      "link": "它与 {addOn} 的连接"
    },
    "updateToo": "一并更新"
  },
  "featurePage": {
    "ask": "管理应用的人员可以安装它。",
    "body": "{count, plural, other {此页面只能配合此应用在这里尚未拥有的附加组件使用，因此不在侧边栏中。附加组件安装并连接到应用后，它就会回来。}}",
    "open": "打开应用的设置",
    "title": "{page} 需要附加组件"
  }
} as const;
