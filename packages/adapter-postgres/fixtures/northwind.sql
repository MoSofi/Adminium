...
-- exact DDL): 14 tables, composite PKs on the join-ish tables, a self-FK
-- (employees.reports_to), classic column types (smallint keys, character
-- varying, character(n), real prices, bytea images) and NO defaults/serials —
-- exactly like the well-known pgfoundry port. Data rows are a representative
-- subset (~5–20 per table, ~50 order_details) so CRUD demos look alive;
-- row-count-dependent fields (rowCountEstimate, sizeBytes, activity) are
-- volatile and normalized by the introspection tests.
...
CREATE TABLE customer_demographics (
    customer_type_id character(5) NOT NULL,
    customer_desc text,
...
CREATE TABLE region (
    ...
    region_description character(60) NOT NULL,
...
CREATE TABLE territories (
    ...
    territory_description character(60) NOT NULL,
    region_id smallint NOT NULL,
