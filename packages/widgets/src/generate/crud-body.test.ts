...
/*
 * The `form` block is GONE (plan 50, B9). It was generated by `composeCrudBody`
 * and read by nothing — a second description of the create form that disagreed
 * with the live one about which columns to hide and when to segment. What a page
 * can carry now is `config.form` v2, written only by the form designer and
 * tested in `page-config/crud-form.test.ts`; with no block the dialog derives the
 * form from the reply's live column facts.
 */
describe('composeCrudBody — no form block', () => {
  it('writes none, on a writable table as much as on a read-only one', () => {
    const writable = composeCrudBody(orders.table, orders.classified, ctx());
    expect('form' in writable).toBe(false);

    const noPk = build('public.log_lines', [{ name: 'line', logicalType: 'text', semantic: 'plain' }]);
    expect('form' in composeCrudBody(noPk.table, noPk.classified, ctx())).toBe(false);
  });
});
