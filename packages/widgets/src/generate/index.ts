 ...
 * auto-instantiation pipeline, and nothing else:
 *
 * - **H1/H2** `emitCandidates` + `candidateRules` + the archetype triggers
 ...
 * `page-crud` config body) and `emitDomainDashboardCandidates` (the
 *     per-domain dashboard widget set) — `./crud-body.ts`,
 *     `./dashboard-domain.ts`.
 *
 * WHY IT EXISTS: hooks H1/H2/H4 are in the Engine's hands
 * ("`composeTemplate(templateId, candidates, ctx)` … → `PageConfig` rows written
 * to `adminium_pages`") while also fixes the rules' home in this package ("the
 * Registry owns what can be instantiated and why … so the catalog and its
 * trigger logic never drift apart"). The Engine therefore needs *some* import of
 * this package, allows it exactly one: a leaf subpath that pulls no component
 * code. `@adminium/widgets/page-config` is the wrong one — it is the frozen
 * **config-body schema** leaf that the server and `@adminium/engine/config`
 * bundle, and it must not grow generator logic. So this is a second leaf, under
 * the same rules:
 *
 ...
 * server, which bars from importing this package at all); a parity test pins it
 * to `widgetRegistry`.
 */
