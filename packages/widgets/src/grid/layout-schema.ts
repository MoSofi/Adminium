...
/**
 * Grid-track layout schema surface.
 *
 * The canonical `layoutItemSchema` / `pageLayoutSchema` Zod definitions live in
 * the pure-Zod leaf `./page-config/layout.ts` — that module is import-frozen to
 * `zod` only so `@adminium/engine/config` and the server can consume the schema
 * without pulling React, dnd-kit, or node builtins (leaf-purity.test.ts). The
 * grid cannot be that source (it imports React + dnd-kit), so re-exporting the
 * leaf here keeps ONE canonical schema while giving the grid track — and the
 * layout-persistence track, which references it — a stable `grid/layout-schema`
 * import point per the wave brief. There is deliberately no second Zod
 ...
 * a widget can never be resized below `minW × minH`. The grid takes this as an
 * injected resolver (`getSizing`) rather than importing the registry, so the
 * grid stays free of widget component code / family chunks.
 */
