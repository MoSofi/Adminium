...
/**
 * Offline asset policy for the widget registry.
 *
 * The maps row, in full:
 *
 ...
 * ATTEMPTED (smoke test asserts zero of them), the user would see an error state
 * where a working visual belongs, and the guarantee would depend on a component's
 * catch block rather than on the app never asking for a map. Resolving one id to
 * another BEFORE the lazy ref is read means the map chunk is never imported, so
 * Leaflet is not loaded — it is not even requested.
 *
 * `map-*` IS A PREFIX RULE, NOT A TWO-ENTRY TABLE, and that is deliberate: the
 * annex family is open, and a future `map-heat`/`map-routes` would otherwise ship
 * a map engine into the offline build on the day it landed, silently and with
 * every gate green. The rule reads the way states it, so new map ids are covered
 * by default and opting one OUT is the change that has to be argued for.
 *
...
/** Which wrapper is running the SPA (detection contract). */
export type WidgetRuntime = 'browser' | 'desktop';

export interface WidgetRuntimeEnv {
  /** `desktop` ⇒ the Electron shell, which must never touch the network. */
  runtime: WidgetRuntime;
  /**
   * Force the offline policy on a non-desktop runtime — second trigger.
   * Self-host behind an air-gap is the case that exists today; the offline smoke
   * test is the case that exercises it without an Electron shell.
   */
...
/** The `map-*` family. Every id under it is network-bearing until proven otherwise. */
export const MAP_WIDGET_PREFIX = 'map-';

/**
 * `choropleth-grid` — the geo family's designed offline fallback: an SVG
...
/** The two triggers, in one place so they cannot drift between call sites. */
export function offlineAssetsRequired(env: WidgetRuntimeEnv): boolean {
  return env.runtime === 'desktop' || env.offlineAssets;
}

/**
 * The registry id a runtime should actually mount for a stored id.
 *
