 ...
 * (annex per-widget `Config` lists). Pure module — no chart primitives, no
 * React — so the schemas + their per-widget default empty copy can be
 * validated independently of the (green-loop-assembled) component barrel.
 * Every schema `.extend()`s the shared config and defaults a per-widget
 * `emptyState` pointing at the i18n copy keys.
 */
import { z } from 'zod';
...
/**
 * Per-widget empty-state copy default (per-widget empty copy). The default is
 * a fully-resolved object because `ZodDefault` returns the default verbatim
 * without re-parsing nested field defaults. Values are `ui` bundle key paths,
 * resolved by WidgetFrame's empty-state translator; static full-key literals
 * on purpose — the i18n coverage guard only counts statically visible
 * `widgets.*` refs.
 */
...
export const chartMultilineConfigSchema = widgetSharedConfigSchema.extend({
  /** Series label at each line's end (annex `seriesLabels`). */
...
export const chartStreamConfigSchema = widgetSharedConfigSchema.extend({
  /** Basis-spline vs straight bands (annex `curve`). */
...
export const chartForecastConfigSchema = widgetSharedConfigSchema.extend({
  /** Steps projected beyond history (annex `horizon`). */
  horizon: z.number().int().min(1).max(120).default(12),
  /** Confidence level for the band (annex `confidence`, e.g. 0.9). */
  confidence: z.number().min(0.5).max(0.999).default(0.9),
  /** `provided` uses the payload's forecast/band; `linear` recomputes (annex
   * `method`). */
...
export const chartAnomalyConfigSchema = widgetSharedConfigSchema.extend({
  /** z-score threshold (annex `sensitivity`). */
...
export const chartCandlestickConfigSchema = widgetSharedConfigSchema.extend({
  /** Corner "live" pill (annex `livePill`). */
  livePill: z.boolean().default(false),
  /** Number of candles to show (annex `candleCount`). */
...
export const chartBumpConfigSchema = widgetSharedConfigSchema.extend({
  /** Override the number of periods; else derived from the payload (annex
   * `periods`). */
  periods: z.number().int().min(2).max(60).optional(),
  /** Override the worst rank shown; else derived (annex `maxRank`). */
...
export const chartTimelineLanesConfigSchema = widgetSharedConfigSchema.extend({
  /** Explicit lane order; else derived from the events' categories (annex
   * `lanes`). */
  lanes: z.array(z.string()).max(12).optional(),
  /** Number of time-axis tick captions (annex `axisBuckets`). */
