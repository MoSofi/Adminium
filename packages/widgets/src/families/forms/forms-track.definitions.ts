...
/**
 * TRACK FCS — `forms` family registry metadata (annex). Metadata only:
 * components load through the `forms-track-components` barrel via `lazy(() =>
 * import(...))`, so the family stays in ONE lazy chunk and the registry
 * metadata never eagerly pulls component code — or Radix's dialog/radio/switch
 * — into a sibling family's bundle (the chunk-budget gate). Schemas +
 * `demoData` come from the PURE `forms-config.ts` for the same reason. The
 * GREEN LOOP spreads `formsTrackDefinitions` into the registry map. Widget ids
 ...
 *
 * SHAPE CHOICES: the input widgets take `form-state` — the envelope for "field
 * defs + values", which is exactly what a generated create/edit form is.
 * `stepper`, `option-cards`, `filter-chip-bar` and `validation-issues-list`
...
export const filterChipBarDefinition: WidgetDefinition = defineWidget({
  ...
  // itself rather than a pre-computed facet payload (annex).
 ...
 * `form-state`, not `record-list` — even though each carries a list. The envelope’s
 ...
 * `form-state` is never empty by, which is exactly right for a canvas.
 */
...
/**
 * Every `forms` widget, in annex order — the TRACK FCS slice (M7 Wave 3) plus the M7
 * Wave-4 TAIL that completes the family (TRACK FORMS): the two builder canvases, the
 * two Studio-wizard widgets lifted out of `apps/dashboard` (see
 * `ConnectionStringField.tsx` / `TableInclusionChecklist.tsx`), the import wizard's
 * column mapper, the export builder, the survey editor and the document-canvas inline
 * field.
 */
