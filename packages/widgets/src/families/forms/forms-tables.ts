...
/**
 * The table-inclusion RULES behind `table-inclusion-checklist` (annex) —
 * PURE module (React-free, JSX-free, copy-free).
 *
 * LIFTED FROM THE STUDIO WIZARD, same reasoning as `forms-dsn.ts`: these rules
 * were born in `apps/dashboard/src/studio/connect/wizardState.ts`, and the
 * wizard now consumes them instead of keeping a rival copy. The rule that
 ...
 * dependency, and the widget binds a `record-list` instead — so the two meet at
 * `InclusionTable`, which is all the RULES ever needed to see.
 */
...
export interface InclusionTable {
  ...
  /** How many of its columns the classifier flagged as PII (annex). */
  piiColumns: number;
  highVolume: boolean;
  /** Join/system tables are pre-hidden — never listed as includable. */
  preHidden: boolean;
  /** Suggested-dashboard tag chip (annex), when the enricher named one. */
 ...
 * (05). Order follows the input so the derived list is deterministic.
 */
...
/** `included`/`total` for the checklist header count (annex). */
