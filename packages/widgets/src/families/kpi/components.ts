 ...
 * family's definitions, so Vite emits one chunk for the whole family. The ONLY
 * module in the family that imports component code: `definitions.ts` reaches
 * it exclusively through `lazy(() => import(...))`, which keeps the
 * @adminium/ui-heavy widgets out of the registry's eager graph (enforced by
