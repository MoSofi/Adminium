 ...
 * can consume it without pulling React/component code (enforced by
 * dep-cruiser + test/leaf-purity.test.ts).
 */
...
// Offline asset policy: `map-*` → `map-choropleth-grid` when
...
// LLM allow-lists — closed vocabularies the
...
// Dashboard grid: static renderer + geometry (M4), edit mode
...
// A host app's own widgets (a project's `widgets/*.tsx` cards), after the registry.
export {
  ExternalWidgetsProvider,
  useExternalWidgetResolver,
  type ExternalWidgetResolver,
  type ExternalWidgetsProviderProps,
} from './frame/ExternalWidgetsContext.js';
export { ChartDirectionBridge, type ChartDirectionBridgeProps } from './frame/ChartDirectionBridge.js';

// Tables family — standalone components + column-spec vocabulary.
...
// Domain family — the annex props-only VIEWS the Studio/platform surfaces
...
// forms family below. `ApiKeysPanelView` backs the API Keys page and
...
// Page templates — page-crud: template component + CrudApi contract.
export * from './templates/page-crud/index.js';
// page-record: the record detail page + host seams.
export * from './templates/page-record/index.js';

// Page templates — planning archetypes: board, calendar, scheduler.
...
// Page templates — the M7 people/queues archetypes.
...
// Page templates — page-log-viewer / page-files / page-chat.
...
// Page templates — page-builder: renderer + doc algebra. `isBlockId`
...
// Page templates — page-wizard: the stepped-flow shell the
...
// Forms family — the two widgets the Studio connect wizard COMPOSES, plus
