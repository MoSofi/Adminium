...
/**
 * The font half of the offline guarantee.
 *
 * The fonts row:
 *
 * > Manrope, JetBrains Mono, IBM Plex Sans Arabic ship as self-hosted woff2 in
 * > `@adminium/tokens`. CJK (zh_CN/zh_TW) uses the > system-font fallback
 * stack — no multi-MB CJK font > download, ever.
 *
...
/** The three, and only these three. */
...
describe('the fonts row — the three families are self-hosted woff2', () => {
  it('declares @font-face for exactly Manrope, JetBrains Mono and IBM Plex Sans Arabic', () => {
    ...
    // approved the download size of, or the CJK bundle rules out. Both are
  ...
  it('vendors no font that fonts.css does not declare', () => {
    ...
    // weight in every installer or — the case this cares about — a CJK bundle
...
describe('the fonts row — CJK is system fonts, never a bundle', () => {
  const SYSTEM_CJK = {
  ...
  for (const [lang, expected] of Object.entries(SYSTEM_CJK)) {
    it(`${lang} names only system CJK faces and ends in a generic`, () => {
      ...
      // THE ASSERTION ACTUALLY MAKES ("no multi-MB CJK font download, ever").
      // Bundling Noto Sans SC is the open decision own list (#5) — it
