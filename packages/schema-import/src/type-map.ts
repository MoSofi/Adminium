...
/**
 * Generic SQL-ish native type → LogicalType mapping ("extend, never fork").
 * Covers Postgres, MySQL/MariaDB and SQLite spellings in one table because
 * imported DDL is dialect-tolerant by design. TypeORM column `type` strings
 * are the same vocabulary, so that parser reuses this too.
 */
