...
export interface AllowedVocabularies {
   ...
   * the referential check (§7.3) is where an invented one is caught, warned
   * about and replaced with `table`. It travels with the other vocabularies
   * because it reaches the server the same way they do — as DATA, since the
   * server tree may not import `@adminium/ui` (01 §2.3) — and because a caller
   * that has one of these lists has all of them.
   *
