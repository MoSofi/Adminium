 ...
 * the injected allow-lists, and the builder's OUTPUT (`PromptArtifact`).
 ...
 * The wizard lets the user deselect groups (all on by default); a
 ...
 * The builder keeps the original numbering (never renumbers) so the
...
// ─── Allow-lists injected into the prompt (builder notes) ────────────────────
 ...
 * cannot drift). They are INJECTED rather than imported here to keep
...
export interface AllowedVocabularies {
  ...
  /**
   * `LUCIDE_ICON_NAMES` — every icon name the dashboard can actually resolve
   * (`@adminium/ui`, generated by `scripts/gen-icon-core.mjs`).
   *
   * Not part of the prompt either: the model is told to emit a lucide name and
   * the referential check is where an invented one is caught, warned about and
   * replaced with `table`. It travels with the other vocabularies because it
   * reaches the server the same way they do — as DATA, since the server tree
   * may not import `@adminium/ui` — and because a caller that has one of these
   * lists has all of them.
   *
   * Optional, like `widgetContracts`: a snapshot bundled before icon names
   * existed still loads, and the icon check simply stays off, which is what it
   * always did.
   */
 ...
 * appears anywhere in the prompt (acceptance criterion 8). When set, the
 * sampling block adds up to `maxValuesPerColumn` most-common values + ordered
 * min/max — but NEVER for PII-suspected or secret columns.
 */
...
/** Provider-agnostic input-token budget per prompt. */
...
export interface BuildPromptOptions {
  ...
  /** Input-token budget for the assembled user section. Default {@link
...
export interface PromptChunk {
  ...
  byo: string;
  /** `ceil(chars / 3.6)` over the flattened document. */
