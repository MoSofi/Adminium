...
/**
 * Shared `@adminium/llm` contract types: the normalized `EnrichmentSet` both
 * paths reduce to, the provider identifiers, and the run status machine.
 *
 ...
 * Re-exported here so every `@adminium/llm` module names the model from one
 * place; the referential checks and the heuristic→EnrichmentSet normalizer both
 * consume it. Type-only: no runtime edge to `@adminium/engine`, keeping this
...
/**
 * `adminium_llm_runs.status` states. Transitions: draft →
 * running | awaiting_response running | awaiting_response →
 * validated | failed | discarded validated → applied |
 * partially_applied | discarded applied | partially_applied |
 * failed | discarded → (terminal)
 */
...
/** States a run can never leave ("Runs are immutable after `applied`"). */
...
// ─── EnrichmentSet — normalized shape ────────────────────────────────────────
 ...
 * and a validated `LlmResponseV1` reduce to, so they diff field-by-field.
...
export interface EnrichmentSet {
   ...
   * wants suppressed (`relation_suppressed`). Carried so the diff can raise a
   * `rejects-heuristic` row a human must confirm (acceptance criterion 12); an
   * accepted one never enters a confidence-gated bulk set. The heuristic
