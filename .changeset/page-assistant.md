...
The pages that build documents — Email templates, Invoice templates, Invoices,
Report builder — gain an **Ask** button in the header. It opens an assistant
...
What it saves is a draft — an email template disabled, an invoice and a report
with status `draft` — and every write leaves an audit row naming the session
that proposed it.
